...
await scenario("orphaned pushed branches are reconciled in their own pipeline, and dependents read the reconciled state", async () => {
  ...
  // no proof that no PR exists, so the delete waits on a settled re-lookup.
  check("the orphan brief reads origin by exit status and holds the delete behind a settled PR lookup", (() => { const p = out.calls.find((c) => c.label === "orphan:012-r").prompt; const probe = /git ls-remote --exit-code --heads origin 'task\/012-r'/g; return (p.match(probe) || []).length === 2 && /read its EXIT STATUS rather than its stdout/.test(p) && /Only a `2` means the push never landed: report `not-on-origin`/.test(p) && /still answers neither `0` nor `2`, report `unresolved`/.test(p) && /A lookup that FAILS \(non-zero exit, no JSON\) is an unsettled read, not an empty one/.test(p) && /Before deleting anything, re-run step 1's lookup, held briefly/.test(p) && /only a lookup that ran and found nothing goes to step 3/.test(p) && /EXITING 2, and report `branch-deleted`/.test(p) && !/printing nothing/.test(p) && !/Nothing printed means/.test(p); })(), out.calls.find((c) => c.label === "orphan:012-r").prompt);
  check("the orphan schema binds `not-on-origin` and `branch-deleted` to the exit-2 answer alone", (() => { const d = out.calls.find((c) => c.label === "orphan:012-r").schema.properties.outcome.description; return /`branch-deleted` ONLY when `git ls-remote --exit-code --heads origin <branch>` EXITED 2 AFTER the delete/.test(d) && /`not-on-origin` ONLY when that same command exited 2/.test(d) && /a probe or lookup that failed rather than answered included/.test(d); })());
