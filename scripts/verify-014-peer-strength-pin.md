...
- **The unobservable residue** is therefore narrow, specific, and confined to the fallback raw launches: the claude peer's effort, which its result envelope carries on no path. A helper launch leaves no such residue — the helper's own result reports the strength it applied, with `model` null on the still-unpinned codex side, itself the reading that the model pin did not apply. That reporting is the half 014's powbox ask called for, and it has landed. Do not fail the run for the residue, but do fail for anything the map says is readable and reads wrong.
