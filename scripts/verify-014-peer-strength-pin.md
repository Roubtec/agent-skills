...
- **The unobservable residue** is therefore narrow, specific, and confined to the raw launches: the claude peer's effort, which its result envelope carries on no path. A helper launch has no residue at all — the helper's own result reports both values, which is the reporting half 014's powbox ask called for and it has landed. Do not fail the run for the residue, but do fail for anything the map says is readable and reads wrong.

## Pass/fail

- Codex peer: the stderr header must show `reasoning effort: medium`, matching the pin above; any other level **fails task 014**. `reasoning effort: none` is what an *unpinned* codex launch inherits — it is not what a helper launch produces, since the helper pins effort itself for both providers and defaults it to `high`. The model line must be the container's configured high-capability model from `~/.codex/config.toml`, not a bare default; a bare default model beside a pinned effort is what a helper launch looks like today, so on this rendering it means the pinned raw launch was not what you exercised.
