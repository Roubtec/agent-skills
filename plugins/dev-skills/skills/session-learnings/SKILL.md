...
6. **Write the report.** In ferry mode write it in the temporary worktree; in local mode write it at the chosen path in the current directory, leaving it untracked and unstaged when that directory is inside a repository. Keep it concise but specific enough that a maintainer can convert entries into tasks. Use this structure:
