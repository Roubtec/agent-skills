...
5. Resolve each number against the inventory and each explicit path or glob using the consumer's existing path/glob semantics. Number inputs select only well-formed task specs in the resolved task subtree. Explicit paths and globs retain the consumer's existing reach: include every existing well-formed task file they would already have selected, including one outside the resolved task subtree. Deduplicate the resulting paths, but attach every selecting raw input to each path as `{ raw, kind }`, where `kind` is `number`, `path`, or `glob`. This provenance is authoritative for consumer policy: consumers must not re-resolve an input to decide how a selected path entered the set.
