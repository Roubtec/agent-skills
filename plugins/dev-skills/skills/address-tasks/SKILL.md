...
- Remove each task's worktree once its PR is open (or once you've decided to stop on it). First verify it is safe: `git -C <worktree> status --porcelain` must be empty and no rebase/merge may be in progress. Then `git worktree remove <path>`. **Never force-remove a worktree with uncommitted changes or an in-progress rebase/merge** — `git worktree remove --force` destroys that work; if the checks fail, leave the worktree in place and report why rather than deleting evidence. Use `--force` only to clear git's refusal over leftovers like ignored build artifacts *after* the clean checks pass. If a `wt-remove` helper is on PATH, prefer it — it encodes exactly these checks.
