...
- "Do not push and do not fetch. Resolve only conflicts the skill classifies as trivial. On the first non-trivial conflict or unrecoverable validation failure, use the unattended clean-stop behavior: after a combined-replay validation failure restore every guide branch in that run to its snapshot; after a per-branch replay restore only the current guide branch to its pre-rebase ref. Leave the worktree clean and stop without waiting for input."
