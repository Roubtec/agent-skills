...
3. **Before delivery, run the sibling add/add collision guard below** across the wave's reviewed-passing branches. For non-colliding tasks, push and open a PR (see Delivery), then remove the task's worktree per Cleanup to reclaim storage (the branch and its commits persist in `.git` and on the remote). For colliding tasks, do **not** open the PR yet; leave the worktree in place, reconcile the naming/path conflict, regenerate derived files, and re-review that task before delivery.
