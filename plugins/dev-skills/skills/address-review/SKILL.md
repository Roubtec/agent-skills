...
2. **Push:** if the expected remote tip is an ancestor of `HEAD`, use a normal explicit push (`git push <remote> HEAD:refs/heads/<headRefName>`). If history was rewritten, use an exact lease (`git push <remote> --force-with-lease=refs/heads/<headRefName>:<expected-head-oid> HEAD:refs/heads/<headRefName>`). If the lease is rejected, **never** escalate to bare `--force`; stop and report because the remote moved under you. Confirm what actually landed against the ref itself — `git ls-remote "$(git remote get-url --push <remote>)" refs/heads/<headRefName>` — rather than `gh pr view --json headRefOid`, which can still report the pre-push head for a while after the push returns. Read back through the resolved **push** URL, not the remote name: where a remote carries a distinct `pushurl` (a fork or a writable mirror), the push wrote to one repository while `git ls-remote <remote>` and any fetch read the other, so the name would confirm the push against a repository it never wrote to.
