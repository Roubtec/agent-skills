...
**The one standing exception to mode 2** is the local off-shoot this skill already supports: an explicit `PR#` was supplied for an **open** PR, and the current checkout is on a named branch that is neither the PR's `headRefName` nor its base ref, is not itself the head of another open PR, and already **carries the PR head** in step 1's exact sense (every commit unique to the head is represented in it, by ancestry or patch-id). That branch is a working location the maintainer chose, so the run works inline on it — fixes land there, `headRefName`/`headRefOid` stay publication metadata, and step 1's reconciliation is skipped rather than used to drag the off-shoot onto the head. A detached HEAD is not covered by this exception: it takes mode 2 whatever it carries.

Two of those conditions exist because a branch satisfying all the others is not an off-shoot at all. `headRefName` is excluded because a ref bearing that name only reaches this paragraph by having failed the identity check — the fork PR's same-named local ref, which carries the head yet resolves its push to `origin`, would otherwise re-enter inline through here, onto the very branch the check refused. Another open PR's head is excluded because a branch stacked on this PR's head satisfies everything else, which is routine in a stacked chain: working there would publish that child's own commits onto this PR under the parent's `headRefName`. One `gh pr list --head <branch> --state open` separates the two, and a PR found there is a stop, not a mode — ask which PR was meant (hands-off: halt naming both branches and both PRs).
...
- Inline (mode 1, the off-shoot exception, and forced `inline`) keeps today's requirement whole: they run on that checkout.
...
- A same-named local ref failing the PR identity check, a candidate off-shoot that is itself another open PR's head, or a reused worktree holding a prior run's dirt or half-finished operation → halt with the report "Working location" defines (both refs and what each points at; both PRs for the stacked branch; or the worktree path and what it holds). Attach nothing, substitute no branch name, clean nothing.
