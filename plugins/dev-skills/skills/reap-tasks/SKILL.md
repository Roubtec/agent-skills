...
In an interactive run, show the resolved mapping before verifying a bare-numbers invocation. Require an explicit continue decision when any number-selected full number is `done`, `deferred`, or `ambiguous`, or when any input is `not-found`: a `done` entry remains skipped as already reaped, and its confirmation asks only whether to proceed with the remaining selected work; it must never offer to re-verify that done entry. A `deferred` entry needs explicit inclusion. For each `ambiguous` number, ask the maintainer to select exactly one candidate or exclude that number — a blanket continue must never verify every candidate. Explicit path/glob selections remain executable while this decision is made.
