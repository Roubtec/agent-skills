...
On every review round while available, launch the peer in the background at the same moment as the own reviewer, from the committed task worktree: `codex exec --sandbox read-only --cd <worktree> -o <outfile> -c mcp_servers={} "<prompt>" < /dev/null 2> <stderr-file> &`. Keep the stderr file peekable because progress is emitted there, use the configured high-capability model and request `-c model_reasoning_effort=high` when high/xhigh effort is not otherwise known; when the override is needed, combine the flags as `codex exec ... -c mcp_servers={} -c model_reasoning_effort=high ...`. Allow a loose timeout of about 12 minutes with discretion to wait longer when progress is visible. On timeout or transient failure, retry once, then forfeit only that round; an auth or usage failure on a classify-at-first-invocation path disables the peer for the rest of the run.
