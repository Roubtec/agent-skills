...
Before launching Phase A, topologically partition rebase-enabled stack entries into dependency waves. Launch only entries whose parents have completed Phase A at a stable tip; after each parent completes, pin each dependent's rebase target to that exact resulting parent tip, then launch the newly ready wave, while unrelated and non-rebase entries may still run concurrently. If a required parent was skip-and-recorded during setup, fails to produce a stable completed Phase-A tip, or is later blocked at the 12-round cap, skip-and-record the rebase-enabled dependent — even if its own Phase A already ran — and recursively affected descendants, naming the parent and its reason; never fall back to the original pinned target.
...
Record the entry's exact `HEAD` before the invocation, and tell the agent to make no PR mutations and to return the complete review packet defined by `address-review` when a requested rebase changes that tip even if no review items remain; that case is a zero-item packet, not a terminal no-op.
If it reports a successful no-op because no actionable review items remain, mark that entry complete without a reviewer or publisher only when its final `HEAD` equals the recorded tip, and treat that unchanged-tip completion as satisfying each dependent's topological gate. If the tip changed, including through the requested rebase, require a complete zero-item packet and send the entry through Phase B and, unless `no-push`, the normal publication waves.

### Phase B — fresh review

Only after all Phase-A agents return, spawn one fresh reviewer per PR and, at the same moment for each entry, launch a background `codex` peer unless disabled or unavailable.
...
For each detected rebase-enabled stack, publish in topological base-before-dependent waves: launch a dependent's publisher only after every parent has published successfully, refreshed as merged, or completed Phase A as an unchanged-tip zero-feedback no-op, and block the dependent if a required parent publication fails. A changed-tip zero-item parent does not satisfy this shortcut: it must pass review and publish first. Publishers for unrelated entries may still run concurrently.
Immediately before that launch, re-fetch the PR and relevant refs and re-verify the same state, pair, and OID facts again. Invalidate the stale packet only when an entry-affecting fact changed: its head OID, open/merged state, fully qualified head/base pair identity, or dependency target because a parent was just rebased or newly merged; unrelated `baseRefOid` movement alone does not invalidate it. Restack only a rebase-enabled dependent whose parent changed its dependency target. For a newly merged parent, pin the refreshed branch containing the merge to one exact commit and reuse it for every affected dependent; when neither kind of parent changed the dependency target, preserve the original exact non-parent target.
...
- [ ] Stacks detected only by matching fully qualified repository/ref pairs; any enabled rebase ran and published base-before-dependent, with an unchanged-tip zero-feedback parent satisfying the gate but a changed-tip zero-item parent reviewed and published first; dependents of setup-skipped, Phase-A-failed, or round-12-blocked parents were skip-and-recorded with the parent reason rather than rebased onto the original target; every rebase-enabled dependent of a newly merged parent was restacked onto the same exact pinned post-merge target and checked for parent content in its diff.
