...
- **Per-PR guidance comes from `address-review`, but the parallel orchestrator owns the phases.** A fix subagent runs `delegated-fix`; a separate fresh reviewer and concurrent best-effort `codex` peer check the returned packet; fix-up/re-review rounds follow as needed; only an entry that clears both verdicts gets a `publish-reviewed` subagent. The peer wiring in standalone `address-review` is not inherited because `delegated-fix` stops before review.
...
For each reviewer round, fan out one same-phase `general-purpose` `Agent` per distinct worktree concurrently and launch that entry's peer beside it, wait for both verdicts for every entry, then advance the phase.
...
Launch from the entry's worktree with unique per-entry/per-attempt files so concurrent output cannot interleave: `codex exec --sandbox read-only --cd <worktree> -o <outfile> -c mcp_servers={} -c model_reasoning_effort=high "<prompt>" < /dev/null 2> <stderr-file> &`. Use a loose roughly 12-minute timeout, extending it when visible stderr progress or review size warrants; retry a timeout or transient failure once, then forfeit that entry's opinion for the round. An auth/usage failure on a classify-at-first-invocation attempt makes the peer unavailable for all later entries and rounds. Wait for both the Reviewer and peer before deciding this entry's outcome.

Read only both verdict lines for orchestration. Unintelligible peer output is a non-blocking forfeit. A round passes only when the Reviewer passes and an intelligible peer has no unaddressed grounded findings, whether tagged `blocking` or `minor`. Only when the Reviewer passed and the peer alone would gate, cheaply spot-check each gate-deciding `file:line` and claim; discard nonexistent or self-evidently false references and note the discard. Do not summarize, merge, or rewrite either result.

### Fix-up rounds

For each failed entry, spawn a fresh fix-up agent with that PR's packet and the complete outputs verbatim as separate `Reviewer findings` and `Peer (codex) findings` blocks, omitting only a forfeited peer block.
...
Then spawn a fresh reviewer and peer round; the next Reviewer adjudicates any evidence-backed push-back on a peer claim.
...
- [ ] Per-PR phases ran in order: `address-review ... delegated-fix`, fresh external Reviewer plus concurrent best-effort peer, fresh fix-up/re-review as needed (6 reviewer rounds max), then `address-review ... publish-reviewed` only after both outcomes cleared the gate unless the run is `no-push`; distinct heads fanned out concurrently but throttled; same-head entries serialized.
