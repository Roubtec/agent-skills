...
For each detected rebase-enabled stack, publish parent-before-dependent as a **best-effort ordering, not a hard gate**: launch a dependent's publisher only after every parent has published successfully, refreshed as merged, or completed Phase A through the terminal zero-feedback shortcut with `final HEAD == starting HEAD == recorded PR headRefOid`. A changed-tip or already-unpublished zero-item parent does not satisfy this shortcut: it must pass review and publish first. Publishers for unrelated entries may still run concurrently.

A parent that *cannot* publish — blocked at the 12-round cap, holding a blocker, or awaiting a maintainer decision — blocks only itself. Publish a dependent that cleared its own review gate anyway, onto the parent's existing remote tip, and reply and resolve its threads normally: the fixes are on origin, so the thread state is honest. Holding the dependent instead is the more fragile choice, because its reviewed work then survives only in a worktree that teardown may remove and a later rebase may strand, while its disposition context lives only in this session. Do not publish the parent's un-gated work on the dependent's behalf. Do not merge a dependent before its parent — an ordering this skill can only state, never enforce — and do not read any of this as discouraging a dependent PR that deliberately targets a feature branch rather than the default.

**Ping only on an honest diff.** Publishing a dependent while its parent's rebase is still unpublished leaves the dependent's PR diffed against a base ref its local history no longer descends from, so the PR shows every commit the rebase pulled in rather than the dependent's own change. Test it mechanically: the dependent's base-ref current remote tip must be an ancestor of the tip just pushed. When it is not, complete the push, replies, resolves, and Summary comment, but **ping no reviewer** — a review round against a sprawling diff is wasted effort — and say so prominently in that PR's Summary comment and in the batch report, including that the diff narrows once the parent publishes or at the pre-merge rebase onto the default branch, and that re-requesting review is then the maintainer's to trigger. An explicit `ping-*` argument overrides this suppression; the bare default respects it.

A parent refreshed as **merged** satisfies a dependent's ordering only when the merge actually carries the parent's final reviewed content. A parent merged after its review gate but before its publisher ran holds reviewed fix-up commits that never reached origin, and restacking the dependent onto that merge then either drops them or silently replays them into the dependent's PR — either way the batch reports success while the parent merged without its fixes. Verify that every commit unique to the parent's final reviewed tip is represented in the refreshed base by patch-id. If any is not, do not treat the parent as satisfied: block that parent, report its unpublished reviewed commits with their SHAs and what each fixed, and leave to the maintainer whether they re-land as their own PR or ride the dependent. The batch never makes that call silently.
...
Tell it to invoke the `address-review` skill with arguments `#N hands-off publish-reviewed <resolved push/ping tokens>` — pass the run's resolved push/ping set (a bare default batch resolves to `ping-contributing`; a `no-push` batch skips publication entirely).
