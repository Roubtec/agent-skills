...
Immediately before that launch, re-fetch the PR and relevant refs and re-verify the same state, pair, and OID facts again. Any changed fact invalidates the stale packet and requires validation/review again; restack only a rebase-enabled dependent whose just-rebased or newly merged parent changed its dependency target, otherwise preserve the original exact non-parent target.
...
- [ ] Stacks detected only by matching fully qualified repository/ref pairs; any enabled rebase ran and published base-before-dependent, and every rebase-enabled dependent of a newly merged parent was restacked and checked for parent content in its diff.
