...
5. Run the per-PR **fix → review → fix-up** loop, up to 12 reviewer rounds, in topological waves for rebase-enabled stacks: a parent must reach its final review-gated tip before its dependent starts Phase A, while unrelated entries can still advance concurrently (see "Per-PR phased subagents").
...
At every apply boundary — immediately before Phase A, each later dependency-wave launch, and each fix-up — re-fetch each affected PR's current state and relevant remote refs, then re-verify its open/merged state, fully qualified head/base pairs, and OIDs. Recompute stack ordering from those refreshed facts, but preserve the original exact non-parent target; replace a dependent's target only when a just-rebased or newly merged parent changes that dependency target. For a newly merged parent, resolve the refreshed branch containing the merge to one exact commit at that boundary and assign that same pinned SHA to every affected dependent.
...
Immediately before that launch, re-fetch the PR and relevant refs and re-verify the same state, pair, and OID facts again. Invalidate the stale packet only when an entry-affecting fact changed: its head OID, open/merged state, fully qualified head/base pair identity, or dependency target because a parent was just rebased or newly merged; unrelated `baseRefOid` movement alone does not invalidate it. When a packet is invalidated, do not launch its publisher: rerun delegated-fix (including the required restack only when a rebase-enabled dependent's parent changed its dependency target) and the fresh review gate first. For a newly merged parent, pin the refreshed branch containing the merge to one exact commit and reuse it for every affected dependent; when neither kind of parent changed the dependency target, preserve the original exact non-parent target.
...
- [ ] Stacks detected only by matching fully qualified repository/ref pairs; any enabled rebase ran and published base-before-dependent, with only an unchanged local tip equal to the recorded PR head eligible for the terminal zero-feedback shortcut, while changed or already-unpublished zero-item parents were reviewed and published first; dependents of setup-skipped, Phase-A-failed, or round-12-blocked parents were skip-and-recorded with the parent reason rather than rebased onto the original target; every rebase-enabled dependent of a newly merged parent was restacked onto the same exact pinned post-merge target and checked for parent content in its diff.
