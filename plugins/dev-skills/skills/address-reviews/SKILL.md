...
- [ ] Stacks detected only by matching fully qualified repository/ref pairs; any enabled rebase ran and published base-before-dependent, with an unchanged-tip zero-feedback parent satisfying the gate but a changed-tip zero-item parent reviewed and published first; every rebase-enabled dependent of a newly merged parent was restacked onto the same exact pinned post-merge target and checked for parent content in its diff.
