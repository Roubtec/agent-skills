...
For each detected rebase-enabled stack, publish in topological base-before-dependent waves: launch a dependent's publisher only after every parent has published successfully, refreshed as merged, or completed Phase A through the terminal zero-feedback shortcut with `final HEAD == starting HEAD == recorded PR headRefOid`, and block the dependent if a required parent publication fails. A changed-tip or already-unpublished zero-item parent does not satisfy this shortcut: it must pass review and publish first. Publishers for unrelated entries may still run concurrently.
...
- [ ] Stacks detected only by matching fully qualified repository/ref pairs; any enabled rebase ran and published base-before-dependent, with only an unchanged local tip equal to the recorded PR head eligible for the terminal zero-feedback shortcut, while changed or already-unpublished zero-item parents were reviewed and published first; dependents of setup-skipped, Phase-A-failed, or round-12-blocked parents were skip-and-recorded with the parent reason rather than rebased onto the original target; every rebase-enabled descendant of any changed parent tip was invalidated, restacked in topological order onto exact parent tips, re-reviewed, and checked for parent content in its diff.
- [ ] Each entry resolved to a `(branch, PR#)` pair and checked out on the right ref — both branch and PR-number resolution sourced the base repository with `gh repo view --json nameWithOwner`; **branch entries use the local ref, never `origin`**; PR-number entries prefer a same-named local branch, else `origin` head; worktrees under the chosen base dir; un-setup-able / PR-less entries skipped-and-recorded.
