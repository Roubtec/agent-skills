...
If it reports a successful no-op because no actionable review items remain, mark that entry complete without a reviewer or publisher only when its final `HEAD` equals the recorded tip. If the tip changed, including through the requested rebase, require a complete zero-item packet and send the entry through Phase B and, unless `no-push`, the normal publication waves.
...
For each detected rebase-enabled stack, publish in topological base-before-dependent waves: launch a dependent's publisher only after every parent has either published successfully or refreshed as merged, and block the dependent if a parent publication fails. Publishers for unrelated entries may still run concurrently.
Immediately before that launch, re-fetch the PR and relevant refs and re-verify the same state, pair, and OID facts again. Any changed fact invalidates the stale packet and requires validation/review again; restack only a rebase-enabled dependent whose just-rebased or newly merged parent changed its dependency target. For a newly merged parent, pin the refreshed branch containing the merge to one exact commit and reuse it for every affected dependent; when neither kind of parent changed the dependency target, preserve the original exact non-parent target.
...
- [ ] Stacks detected only by matching fully qualified repository/ref pairs; any enabled rebase ran and published base-before-dependent, and every rebase-enabled dependent of a newly merged parent was restacked onto the same exact pinned post-merge target and checked for parent content in its diff.
...
- [ ] PR/merge state and relevant refs refreshed and re-verified before Phase A, every later dependency-wave launch, each fix-up, and publication; refreshed facts preserved the pinned non-parent target and changed only a just-rebased or newly merged parent's dependency target.
- [ ] Per-PR phases ran in order: `address-review ... delegated-fix`, fresh external Reviewer plus a concurrent best-effort peer when enabled and available, fresh fix-up/re-review as needed (12 reviewer rounds max), then `address-review ... publish-reviewed` only after the Reviewer passed and the peer outcome satisfied the conditional gate unless the run is `no-push`; the zero-feedback shortcut was used only when Phase A left the tip unchanged, while a changed-tip zero-item packet passed review and, unless `no-push`, publication; distinct heads fanned out concurrently but throttled only for objective or anticipated constraints; same-head entries serialized.
