...
function rebasePrompt(point, packet, target, explicitTarget) {
  ...
  const pin = explicitTarget
    ? `The target is \`${target}\`, named outright by this run's request rather than taken from the PR. Resolve it WHERE IT WAS NAMED — here, in this working location — with \`git rev-parse --verify ${shq(`${target}^{commit}`)}\` — the operand quoted as ONE argument exactly as rendered, because a ref name may legally carry characters the shell would otherwise expand or split (\`$\`, backticks and \`;\` all pass \`git check-ref-format\`), and an unquoted \`$\`-fragment can silently resolve a DIFFERENT existing ref instead — which takes a local branch, a remote-tracking ref, or an exact commit, or a tag: \`git rev-parse\` prefers \`refs/tags/\` over \`refs/heads/\`, so a name carried by BOTH resolves to the TAG with only an \`is ambiguous\` warning on stderr — treat that warning as a stop and report it rather than pinning what it resolved to, proves it names an existing COMMIT, and prints the full object id. Fetch NOTHING for it, and do not go looking for it in the PR's base repository: an explicitly named target is routinely a local branch or one in the head fork, so asking that repository for \`refs/heads/<it>\` pins an unrelated same-named branch there, or fails outright on a target that was on disk all along. Where it does not resolve here, report \`ok: false\` naming what you tried, and substitute nothing.`
