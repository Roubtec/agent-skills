...
A thread asking you to DOCUMENT some behavior is satisfied by a minimal why-comment under the cycle's comment rule, by the reply your \`detail\` carries on the thread, or by \`push-back\` — never by adjacent code re-implemented in prose. Where such a push-back is sustained, that rule's carve-out for a standing overruled decision is worth the comment precisely here: an external reviewer re-raises the same point across PR rounds and runs, and this run's replies are not in front of it next time.
...
function publishPrompt(packet, dispositions, flags, deviations, recordOnly) {
...
phase("Publish");
const publishReport = await agent(publishPrompt(packet, workReport, publishFlags, cycle.deviations, cycle.recordOnly), {
  label: "publish",
