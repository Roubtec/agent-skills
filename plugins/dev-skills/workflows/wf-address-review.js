...
const partialReruns = workReport.filter((d) => isRerunOwed(d) && (accountByKey.get(dispKeyOf(d)) || {}).rerun !== true && acceptedRunIds(d).length).length;
...
const standingLine = (d) => {
  ...
  if (!isReviewThreadOwed(d)) {
    ...
    if (isRerunOwed(d)) {
      if (o && o.rerun === true) return `${label} — flake re-run ALREADY ORDERED (every run id the lane names accepted) — do not order it again; nothing else is owed on a lane (no thread to reply on or resolve).`;
