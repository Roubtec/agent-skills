...
1. Re-check before publication: clean worktree, no rebase in progress; re-fetch the PR and confirm it is still open and still points at the expected head repo/ref. Resolve the exact push remote/ref for the PUBLICATION TARGET — the PR's head repository and \`refs/heads/${shq(packet.pr.branch)}\` — and verify the remote you resolved is that repository (never assume \`origin\`, especially for forks). That target is the PR's, not the checked-out branch's: where the branch you addressed is not the head ref (\`workingBranch\` \`${packet.pr.workingBranch}\` against head ref \`${packet.pr.branch}\` — the off-shoot case), the off-shoot's own upstream is NOT the target, so its failing to match the PR head is that case's normal state rather than a stop, and nothing this run pushes goes there. What an off-shoot may put ON the head ref is step 2's representation probe, not a name comparison here. Expected head OID to replace: \`${packet.pr.headOid}\`. If the head moved or the target repository/ref can't be matched, set \`published: false\`, \`aborted\`, and STOP — do not guess.
