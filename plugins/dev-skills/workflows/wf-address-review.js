    ...
    threadOutcomes: {
      ...
      items: {
        ...
        properties: {
          ...
          rerun: { type: "boolean", description: "On a `ci-failure` entry whose disposition is `flake-rerun`, after a NO-OP push: true ONLY if `gh run rerun` exited 0 for EVERY distinct run id the lane names (its url's and every one in its instances). False when any request was rejected or failed, or none was issued. Left out on a push that advanced the branch, where nothing is ordered, and on every other entry. That re-run is the one action the disposition promises, so a completion claim over a flake-rerun lane reporting it false or absent after a no-op push is refused exactly as one over an owed reply that never posted." },
        },
...
const owedReruns = rerunKeys.filter((k) => (accountByKey.get(k) || {}).rerun !== true).length;
...
const contradictedByAccount = unpushedClaim || owedReplies || owedResolves || owedReruns
  ? `its own account contradicts that claim — ${[
      ...
      owedReruns ? `${owedReruns} of ${rerunKeys.length} flake-rerun lane(s) it owes a re-run after its no-op push report no \`gh run rerun\` accepted (\`rerun: true\`) — the one action that disposition promises` : "",
...
const standingLine = (d) => {
  ...
  if (!isReviewThreadOwed(d)) {
    ...
    if (isRerunOwed(d)) {
      return o && o.rerun === true
        ? `${label} — flake re-run ALREADY ORDERED (every run id the lane names accepted) — do not order it again; nothing else is owed on a lane (no thread to reply on or resolve).`
