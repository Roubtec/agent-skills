...
[<disposition>] <path>:<line>  <author>  thread=<threadId, or url (finding N of M) for a standalone item, or lane for a CI lane>
                url:   <permalink>
                finding: "<a misfired finding only: its text as the gather took it, verbatim>"
...
The text each misfired finding's \`finding:\` line carries, VERBATIM — one per standalone item the gather took from a bot's misfired top-level comment, keyed by its url and ordinal:
