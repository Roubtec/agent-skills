  ...
  properties: {
    ...
    pr: {
      ...
      properties: {
        ...
        baseOid: { type: "string", description: "The full OID that base ref resolves to right now IN THE BASE REPOSITORY — the repository the PR itself is in, freshly fetched, never read through the branch's push remote, which on a cross-repository PR is the head fork. A commit, never a name. REQUIRED on a `no-rebase` run and ONLY there — that run has no rebase report to pin from, so this is the OID it pins as its review base, and the caller rejects anything that is not a full hex OID on that path and stops the run. A rebasing run never reads it (its rebase pins the base itself), so its gather reports the field EMPTY and performs no base-repository fetch — the brief orders that fetch only where the value is consumed." },
