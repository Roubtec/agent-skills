...
Preclude repeat comments: for each pattern you fix, grep the PR's changed files and closely related code for the SAME offending pattern and fix those too; report them in \`proactive\` as an explicit enumeration of the space you searched with a per-item verdict, never an assertion that the sweep is complete.
