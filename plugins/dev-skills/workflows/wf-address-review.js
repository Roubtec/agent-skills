...
1. Re-check before publication: clean worktree, no rebase in progress; re-fetch the PR and confirm it is still open and still points at the expected head repo/ref. Confirm the working location still stands on the branch this run addressed — \`git rev-parse --abbrev-ref HEAD\` must print \`${packet.pr.workingBranch}\` — because every rule below is keyed on that NAME rather than on the shape of the history, and a checkout that moved to some other branch since the review would be published from with the wrong HEAD; where it prints anything else, set \`published: false\`, \`aborted\`, and STOP without pushing. Then resolve the push target, which is a different question in each case (\`workingBranch\` \`${packet.pr.workingBranch}\` against head ref \`${packet.pr.branch}\`):
