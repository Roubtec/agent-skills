...
// The token itself stays as permissive as its neighbours, because a false
// negative stops the supported case outright. What is narrowed is the TEXT it
// is read out of, the way `pushWords` strips `push-back` before reading `push`:
// three contexts mention an off-shoot without selecting one, and each of them
// occurs in an ordinary request. (1) A ref PATH is a name — `rebase on top of
// task/021c-publication-guard-for-an-off-shoot` is a supported argument form in
// this very repository, and a branch name can hold no space, so stripping the
// slash-bearing word cannot swallow the spaced spelling. (2) A quoted PHRASE is
// something being quoted rather than asked for; a quoted TOKEN survives, so
// `"off-shoot"` still selects. Double quotes and backticks only — the
// apostrophe in ordinary prose ("don't work on the off-shoot, it's fine") pairs
// up and would swallow the token between the two marks. (3) A NEGATED mention
// asserts the opposite of a selection. A false positive here is NOT harmless:
// it disables this guard and admits exactly the deviating
// `workingBranch !== branch` packet the guard was added to reject.
const offShootWords = lower
  .replace(/\S*\/\S*/g, " ")
  .replace(/(["`])(.*?)\1/g, (m, _q, inner) => (/^\s*off[\s-]*shoots?\s*$/.test(inner) ? m : " "))
  .replace(/\b(?:not|never|isn'?t|without)\s+(?:an?|the|any|its|it's)?\s*off[\s-]*shoots?\b/g, " ");
const offShootTok = /\boff[\s-]*shoots?\b/.test(offShootWords);
