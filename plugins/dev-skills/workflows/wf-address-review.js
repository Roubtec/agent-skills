...
If there are no unresolved threads and no included standalone item, return \`ok: true\` with an empty \`items\` array — plus everything a CONTINUING run consumes: \`rebaseTarget\` as above, the three tips, and (on the \`no-rebase\` arm above) \`pr.baseOid\` where the tips disagree. The caller decides between the two zero-item outcomes exactly as \`address-review\` step 3 does, by comparing \`pr.finalHead\`, \`pr.startingHead\`, and the recorded \`pr.headOid\`: all three the same commit and the run finishes as a successful TERMINAL no-op; anything else takes the ZERO-ITEM PATH, continuing through the normal fresh review and, unless \`no-push\`, publication — that tip carries work the PR head does not (an unpublished commit an earlier run left, a tip that moved under this one), and reporting "nothing to address" would leave it unreviewed and unpushed.
...
if (!packet.items || packet.items.length === 0) {
  ...
  if (!startTip || !finalTip || !recordedTip) {
    return {
      ...
      note: "Nothing was addressed and nothing was pushed. Re-run: a zero-item gather must report `pr.startingHead`, `pr.finalHead`, and `pr.headOid`, so the caller can tell a terminal no-op (all three the same commit) from the zero-item path (a tip the PR head does not carry, which continues through review and, unless `no-push`, publication).",
    };
  }
  ...
  zeroItemRun = {
    ...
    why: finalTip !== recordedTip
      ? `no review item is outstanding, but the local tip \`${finalTip}\` is not the recorded PR head \`${recordedTip}\` — work the PR does not carry, which this run reviews and, unless \`no-push\`, publishes rather than reporting "nothing to address" over it`
