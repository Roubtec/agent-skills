    ...
    items: {
      ...
      items: {
        ...
        properties: {
          ...
          rollup: { type: "string", description: "The lane's state on the reconciled head, on a `ci-failure` item: `failing` (its verdict is none of `SUCCESS`/`NEUTRAL`/`SKIPPED`, the ordinary reason a lane is an item), or — ONLY for a lane a prior disposition record holds an entry for — `green` or `running`, the two states in which nothing re-gathers a lane on its own and the item exists so the recorded cause and disposition replay into this run's Summary comment rather than being dropped." },
          path: { type: "string" },
          line: { type: "integer" },
...
const PUBLISH_SCHEMA = {
  ...
  properties: {
    ...
    aborted: { type: "string", description: "Why publication stopped, when published is false (e.g. `head moved`, `working location moved off the branch`, `lease rejected`, `local behind PR head`, `off-shoot does not carry the PR head: <both tips and the probe's commits>`, `push remote unmatched`, `" + PUSH_UNCONFIRMED_ABORT + ": <what each URL returned>`). Empty when published. Write it: the run's own note carries this reason inline, so an omitted one reads as `no reason reported` there." },
...
Empirical verification that could change state belongs ONLY in a disposable clone: work in \`DC="$(dc-enter <slug>)"\` — it prints one absolute path on stdout, \`dc-remove <slug>\` drops it, and a reused slug is REFUSED rather than re-derived, so pass \`--replace\` or remove the slug first if this may run twice. Where that command is not found at all, install the helpers from the dev-skills plugin \`bin/\` rather than improvising a destination. Never \`cd\` into a path held in a variable unguarded: \`cd ""\` returns 0 and moves nowhere, so checking the status catches nothing and a lookup that produced no path leaves you in the shared checkout. Write \`cd -- "\${DC:?dc-enter returned no path — see its error above; if it is not installed, install it from the dev-skills plugin bin/}"\`, and confirm \`pwd\` before the first command that writes.`;
...
Pick the WORKING LOCATION now — after resolving the PR, before reconciling below (reconciling can fast-forward a branch, and it must move the branch the work will land on). Let \`T\` be the PR's headRefName and \`C\` the current branch (\`git branch --show-current\`, EMPTY when detached). Fetch the PR's exact head ref WITHOUT moving any branch and let \`R\` be \`git rev-parse FETCH_HEAD\` — this step's OWN fetch, run HERE in the checkout you start from, since case 4's worktree does not exist yet and \`R\` is what that case creates a missing \`T\` at. It does NOT stand in for the reconciliation's fetch below, which runs wherever this step lands you: \`FETCH_HEAD\` is per-worktree state, kept in each tree's own git dir — \`git rev-parse --git-path FETCH_HEAD\` in a linked worktree prints \`.git/worktrees/<name>/FETCH_HEAD\` — so a worktree this step just attached has no such file at all, and reading one there fails with \`ambiguous argument 'FETCH_HEAD'\` rather than resolving the head you fetched over here. Every tree reads only the \`FETCH_HEAD\` its own fetch wrote; that the two reads land on one file when you work INLINE is no licence to skip the second fetch.
...
function publishPrompt(packet, dispositions, flags, deviations, deviationAssessments, recordOnly, preRebaseRecordOnly) {
  ...
  // WHICH repository, which the working location above does NOT answer: every
  // PR-scoped `gh` call this brief orders is qualified from the PR's own URL,
  // for the reason the record brief's writes already are. On a cross-repository
  // PR handled from a fork clone the checkout IS the head fork, while the PR and
  // its comments live in the base repository — and `gh api`'s `{owner}`/`{repo}`
  // placeholders and a bare `gh pr view`/`gh pr comment`/`gh pr edit` all answer
  // for the current directory. This brief is the one that MUTATES the PR, and it
  // never sees the skill's recipes section where the rule is otherwise stated,
  // so unqualified it would read, reply to, comment on and request a review of a
  // same-numbered PR in the fork. The `git` push path is deliberately not
  // qualified this way: its target is the head repository, resolved from the
  // remote in item 1.
...
WHICH REPOSITORY every PR-scoped \`gh\` command below addresses: the one this PR is IN, whose \`<owner>/<repo>\` is already resolved and handed to you in the PR's own URL \`${packet.pr.url}\` — never the repository your working location resolves to. On a cross-repository PR the checkout is the HEAD fork while the PR, its threads and its comments live in the base repository, and every default here answers for the wrong one: \`gh api\`'s \`{owner}\`/\`{repo}\` placeholders expand to the repository of the current directory, and a bare \`gh pr view\`, \`gh pr comment\` or \`gh pr edit\` resolves the same way — so unqualified, item 1's head-repository read reads a same-numbered PR in the fork, item 4's reply \`POST\` addresses a comment id there, and item 5's Summary comment and item 6's pings land on that PR instead of this one. So write that \`<owner>/<repo>\` out LITERALLY in every one of them. Do not re-derive it from a bare \`gh repo view --json nameWithOwner\`, which with no repository argument answers for the directory — in a fork clone the head fork, the one repository this paragraph exists to keep these writes away from. The \`git\` commands are the deliberate exception and stay exactly as written: the push, its lease and its read-back address the remote you resolve in item 1, which on a cross-repository PR is the head fork rather than the repository the PR is in.
...
3. Re-read unresolved threads after the push. Do not mutate newly-arrived feedback that was not triaged this run — leave it open and call it out. Then the \`flake-rerun\` lanes among the dispositions below, ONLY where the push was a no-op (\`Everything up-to-date\`): a push that advanced the branch already re-ran every lane, so re-run nothing then. For each, take \`<owner>/<repo>\` and \`<run-id>\` from the lane's details URL (its \`url\`, the \`/<owner>/<repo>/actions/runs/<run-id>\` segment) — the repository from the URL, never the checkout's default, since a fork PR's workflow runs in the base repository — and run \`gh run rerun <run-id> --repo <owner>/<repo> --failed\` ONCE PER RUN ID: several failed jobs of one run share its id and \`--failed\` re-runs them all, and a second request meets a run already re-running. A lane whose url names no workflow run (a \`StatusContext\`, an external app's check) cannot be re-run from here; say so in its Summary line and leave it for the maintainer. This run does not wait on the fresh rollup: CI gates the merge, and the next round reads it.
...
// `push-back` the one it may not: a lane has nobody to push back to, and the
// skill's disposition-by-cause gives a lane this branch did not cause to the
// maintainer as ambiguous, with the cause found, rather than arguing with it.
...
function dispositionDefect(d) {
  ...
  // reads as covering two items while publication serves one.
  const namesLane = typeof d.lane === "string" && laneItemById.has(d.lane.trim());
  const namesStandaloneUrl = typeof d.url === "string" && standaloneUrls.has(d.url);
  const namesThread = !!d.threadId && threadItemById.has(d.threadId);
  if (d.type === "review-thread") {
    ...
    if (!standaloneItemByKey.has(itemKeyOf(d))) {
      ...
      if (standaloneUrls.has(d.url)) return `its url is a gathered misfired comment's, but its finding ordinal ${findingOrdinal(d) ? JSON.stringify(findingOrdinal(d)) : "is absent and"} names no finding gathered from it — every finding of that comment shares the url, so on the url alone this entry would stand for all of them`;
