...
Rebase NOTHING here, whatever the request asked for. Report \`pr.base = baseRefName\` and \`pr.rebased = false\`. Also resolve that base ref to a commit ONCE and report the full OID as \`pr.baseOid\`, reading it in the BASE repository and not through this branch's push remote. Those are two different repositories whenever the PR is cross-repository: the push remote is the HEAD repository, so \`<push-remote>/<baseRefName>\` there names a same-named branch in the fork — a different branch's tip, or nothing at all — and the push remote stays what it is for, the publication target. There is no base-repository field to ask for and none is needed: a PR's base always lives in the repository the PR itself is in, so the base repository is that one (\`gh repo view --json nameWithOwner\`, the same source \`address-reviews\` takes the base side of its pairing from), with \`isCrossRepository\` as the cheap short-circuit — false means the push remote IS that repository and one remote serves both. Fetch that repository's exact base ref WITHOUT moving any branch — \`git fetch <the remote whose URL is that repository, or that repository's URL where a fork clone has no remote for it> refs/heads/<baseRefName>\` — and resolve what the fetch brought: \`git rev-parse --verify FETCH_HEAD^{commit}\`, reporting the full unabbreviated OID git prints. Read no \`<remote>/<baseRefName>\` in its place: a remote-tracking ref is only as fresh as whatever last fetched it, so it can pin a commit the base has since moved past, and nothing else here fetches the base at all. That fetch OVERWRITES \`FETCH_HEAD\`, so run it only after the reconciliation above has taken its \`R\` from its own fetch. A \`no-rebase\` run pins its review base to that OID, because every range this run delegates is taken against a commit rather than a name — a name moves under the next fetch or a sibling push. Then put an explicit \`rebase on top of <branch>\` token's target — a branch name or an exact commit, verbatim — into \`rebaseTarget\` (empty when none was named). A delegated rebase step runs immediately after you and again before publication; it resolves that target to a commit, pins it, and reports back what it landed on, and the caller replaces \`pr.base\`/\`pr.rebased\` from that report. Two agents both rebasing would replay the same commits twice, which is precisely why this one does not.
...
// validation wording differ between them, because the mechanics must not.
function rebasePrompt(point, packet, target) {
...
// conflict, a failed one, a broken build, and an `effectiveBase` that is not a
// commit. That last check is the load-bearing one, and it is a check rather
...
async function rebasePoint(point, target) {
  phase("Rebase");
  const report = await agent(rebasePrompt(point, packet, target), {
    label: `rebase-${point}`,
