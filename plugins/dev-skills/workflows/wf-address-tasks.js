...
const PR_SCHEMA = {
  ...
  properties: {
    ...
    reason: { type: "string", description: "When opened is false: why (no remote auth, gh error, no PR from creation or lookup, a wrong base the repair could not fix — name the base it carries — or a read that did not settle: a by-head lookup or base read-back still answering stale after a re-read, named with what it last returned). Empty when opened." },
  },
