...
- Resolve the contradiction at `:293` in both mirrors. Two acceptable resolutions:
  - Treat a refused sweep the same way as a breadcrumb that changed since the inventory: report the mismatch and, where Git supplies one, the ref's current position — and print no command; or
