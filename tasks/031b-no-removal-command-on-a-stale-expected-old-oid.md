...
- **Distinguish a vanished breadcrumb from a moved one — and both from a replaced one.** The expected-old deletion refuses in three distinguishable shapes, and only the first names a current position. Verified in a powbox container: the first two on 2026-08-06, the third on 2026-08-07 against Git 2.47.3, and reported identically on 2.43 in review.
  - **Moved** — `cannot lock ref '<ref>': is at <actual> but expected <expected>`.
  - **Vanished**, another process having deleted the ref in between — `cannot lock ref '<ref>': unable to resolve reference '<ref>'`, naming neither OID, because there is no current position to name.
  - **Replaced by a dangling symref** — `cannot lock ref '<ref>': reference is missing but expected <expected>`. It too names no current position, but the breadcrumb name still exists, now as a symbolic ref whose target does not, and `for-each-ref` skips a dangling symref, so a re-inventory will not see it either.

  "Report the ref's current position" is therefore unsatisfiable for the second and third races alike, and the report must not claim a mismatch it cannot describe. Those two are not the same outcome, though: a vanished ref is what the sweep wanted anyway, whereas a replaced one is a breadcrumb that survived the sweep with an unexplained symref standing where it was — the one race here that leaves the user something to act on. So the vanished disposition must be assigned from the ref's observed existence and type after the refusal, never from the absence of an OID in Git's message, which the last two races share: `git symbolic-ref --quiet <ref>` succeeds only for the replaced case, and only a genuinely absent ref fails both it and `git rev-parse --verify --quiet <ref>` (which, on a dangling symref, still warns on stderr despite `--quiet` — do not read that warning as an error). `:253` conflates moved and vanished and must be corrected alongside: it says Git refuses "reporting where the ref actually is, when the breadcrumb moved **or vanished** in between", and that is what Git prints for neither of the last two shapes. `:254` already treats the direct-ref-to-symref swap as a deletion hazard `--no-deref` contains, and this bullet leaves that guard untouched — the swap is in scope here only for how the resulting refusal is reported.
- Make step 11's report categories (`:268`) and the print rule agree, so "changed since the inventory" and "refused by the expected-old-OID check" are not given opposite treatment without a stated reason for the difference. If the vanished or replaced case earns its own disposition, add it there rather than folding it into either.
...
- `plugins/dev-skills/skills/prune-branches/SKILL.md:253` — the executed deletion form and its description of the refusal, `:254` — the `--no-deref` guard against a breadcrumb replaced by a symref, whose refusal shape this task reports but whose mechanics it leaves alone, `:167` — the load-bearing classification the second alternative must re-run, and `:290-293` — the printed-command rules this task repairs. Both mirrors are 315 lines and identically numbered as of `885cdee`; re-derive if they have moved.
...
- The vanished disposition is assigned from the ref's observed existence and type, not from Git's message lacking an OID. A breadcrumb replaced by a dangling symref is reported as still present and unexpectedly replaced — never as vanished, which would retire a ref that is still standing.
...
- In a scratch repository, exercise all four races between inventory and cleanup and confirm the report's wording for each matches whichever resolution was adopted — and that any command it does print succeeds when pasted:
