...
- `plugins/dev-skills/workflows/wf-address-review.js`, `wf-address-tasks.js` — `meta` disclosure and `args` passthrough as they adopt the shared cycle.
...
- Peer concurrency is governed solely by 015's throttle: the script that owns the fan-out carries it as state, introduces no cap or floor of its own, and does not substitute a chunked fan-out for it.
