...
- Every deletion path prints tip SHAs; non-Merged deletions get backup refs with documented restore instructions, and every such ref is created and verified **before** its branch is deleted. Creation claims an unused name rather than overwriting an existing one, so a repeat prune of the same branch cannot repoint an earlier run's breadcrumb; a name that cannot be claimed drops its branch from the deletion set.
