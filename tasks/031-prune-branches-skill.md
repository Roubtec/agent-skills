...
1. **Inventory.** List all local branches with tip SHA, upstream state, and the worktree (if any) each is checked out in. Detect the default branch by ref, not by name (`origin/HEAD`, falling back to `gh repo view --json defaultBranchRef`, then common-name heuristics) — it is never a deletion candidate, and neither is the currently checked-out branch of any worktree until that worktree is handled.
...
3. **Classify with minimal effort.** Three buckets:
   - **Merged:** the branch tip is an ancestor of the default branch (the normal case — our repos merge with provenance intact), or a read-only `gh pr list --state merged --head <branch>` lookup confirms a merged PR, or — best effort for squash-merge repos — the default branch's recent commit subjects reference the branch's PR. No patch-id forensics, no deep computation; squash-merge repos simply get less coverage and that is acceptable.
...
5. **Delete locally.** `git branch -D` the confirmed set. For a branch checked out in a linked worktree, remove the worktree first — via `wt-remove` where available (its refusal to discard uncommitted work is exactly the safety we want), else `git worktree remove` without `--force`; if removal refuses because the tree is dirty, the branch moves to Uncertain and is reported, not forced.
6. **Recovery breadcrumbs.** Print the deleted tip SHA for every branch. For every deleted branch that was not in the Merged bucket, also create a backup ref (e.g. `refs/pruned/<date>/<branch>`) and tell the user how to restore (`git branch <name> <sha>`) and how to drop the backups later. Mention that unreferenced commits remain recoverable until garbage collection.

### Arguments

- `no-pull` — keep the default branch where it is (fetch still runs).
- `yolo` — skip the confirmation and perform a best-effort no-work-lost purge start to finish: delete only Merged and Transient branches, create backup refs for all Transient deletions, and report the Uncertain bucket untouched at the end.
...
- In `yolo` mode the listing is still printed for auditability before acting, mirroring rebase-stack's delegated unattended mode.
...
- Merged and Transient buckets are defined exactly as scoped above; Uncertain branches are never deleted without explicit per-run user confirmation, including in `yolo` mode.
- Every deletion path prints tip SHAs; non-Merged deletions get backup refs with documented restore instructions.
...
- Dry-run the skill text against a scratch repo seeded with: a merged-by-ancestry branch, a squash-merged branch (PR-lookup path), a combination-merge test branch, a branch with unique commits, a branch checked out in a dirty worktree, and a `[gone]`-upstream branch — verify each lands in the intended bucket and the confirmation/yolo flows behave as specced.
...
Reviewer checks the safety rules are stated as absolutes (no remote mutation, no forced worktree removal, default/current/keep branches untouchable), that `yolo` cannot reach the Uncertain bucket, that recovery-ref instructions are complete enough to salvage a mistaken deletion, and that classification stays within the "minimal effort" budget rather than accreting squash-merge forensics.
