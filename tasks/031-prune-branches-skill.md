...
1. **Inventory.** List all local branches with tip SHA, upstream state, and the worktree (if any) each is checked out in. Detect the default branch by ref, never by name: `origin/HEAD`, falling back to `git remote set-head origin --auto` (which re-reads the remote's symbolic HEAD advertisement) and then `gh repo view --json defaultBranchRef`. If none of those resolves, **stop and refuse to delete anything** — report that the default branch could not be identified and what to run to fix it. Do NOT fall back to common-name heuristics: guessing `main`/`master` in a repo whose real default is something else (`develop`, `trunk`) leaves the actual default branch eligible for classification and deletion, violating the absolute safety rule below. The default branch is never a deletion candidate, and neither is the currently checked-out branch of any worktree until that worktree is handled.
...
3. **Classify with minimal effort.** Three buckets:
   - **Merged:** the branch tip is an ancestor of the default branch (the normal case — our repos merge with provenance intact), or a read-only `gh pr list --state merged --head <branch>` lookup confirms a merged PR **whose `headRefOid` equals the current local tip**, or — best effort for squash-merge repos — the default branch's recent commit subjects reference the branch's PR. The head-OID match is required, not optional: a head-name lookup alone still finds the historical merged PR when the local branch has gained commits since that PR merged, or when the branch name was reused for new work, and would classify genuinely unique commits as Merged — the one bucket that gets deleted without a backup ref. When the merged PR's head OID does not match the tip, the branch is **Uncertain**, not Merged. No patch-id forensics, no deep computation; squash-merge repos simply get less coverage and that is acceptable.
...
- The default branch is resolved from a ref or the remote's advertised HEAD; when it cannot be resolved the skill refuses to delete rather than falling back to a name guess.
...
- Merged classification via PR lookup requires the merged PR's head OID to equal the branch tip; a branch that advanced past its merged PR, or reused a merged PR's name, lands in Uncertain.
...
- Dry-run a repo whose default branch is neither `main` nor `master` and whose `origin/HEAD` is unset: the skill must refuse to delete rather than heuristically selecting a branch.
