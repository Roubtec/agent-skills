...
1. **Refresh.** `git fetch --prune` is the run's first remote action, before anything is classified or even resolved, so classification runs against origin's true tip, stale remote-tracking refs disappear as part of the same cleanup, and a newly-advertised default branch has the local ref that step 2's `set-head --auto` needs.
2. **Inventory.** List all local branches with tip SHA, upstream state, and the worktree (if any) each is checked out in. Detect the default branch by ref, never by name — and never trust a cached `origin/HEAD` as it stands. `refs/remotes/origin/HEAD` survives a default-branch rename on the remote and the step-1 fetch does **not** update it, so a present-but-stale symbolic ref names a branch that is no longer default and silently leaves the real default eligible for classification and deletion. Run `git remote set-head origin --auto` (which re-reads the remote's symbolic HEAD advertisement) and only then resolve `origin/HEAD`. The step-1 fetch has to come first because `set-head --auto` only rewrites the symbolic ref when the newly-advertised branch already has a local remote-tracking ref; against an unfetched new default it exits non-zero with `error: Not a valid ref: refs/remotes/origin/<name>` and the stale ref survives untouched. **A non-zero exit from `set-head` therefore forbids falling back to the existing `origin/HEAD`:** that ref still resolves, so a "did it produce a ref" test cannot distinguish refreshed from stale. Treat any `set-head` failure — and any inability to run it at all (offline, no remote, no advertisement) — as unresolved and go to `gh repo view --json defaultBranchRef`, which is authoritative over any unrefreshed `origin/HEAD`. If none of those resolves, **stop and refuse to delete anything** — report that the default branch could not be identified and what to run to fix it. Do NOT fall back to common-name heuristics: guessing `main`/`master` in a repo whose real default is something else (`develop`, `trunk`) leaves the actual default branch eligible for classification and deletion, violating the absolute safety rule below. The default branch is never a deletion candidate, and neither is the currently checked-out branch of any worktree until that worktree is handled. Once the default branch is known, pull it to its latest state by default; the `no-pull` argument suppresses that pull (the step-1 fetch stays, it is read-only).
...
5. **Reserve recovery refs first.** Before any deletion, create the backup ref (e.g. `refs/pruned/<date>/<branch>`) for every confirmed branch that is not in the Merged bucket, creating it only where **no ref of that name already exists**, and verify each ref resolves to the expected tip. On a name already taken, disambiguate rather than refuse — append the branch's short tip SHA, then a counter — since the ordinary case is simply pruning the same branch twice on one date, and a bare refusal would trade a lost breadcrumb for a skill that stops working on its second run of the day. A branch whose backup ref still could not be created or verified is dropped from the deletion set and reported. The unclaimed-name requirement is not pedantry, because the example name is not unique by construction: prune the same branch name twice on one date and a plain ref write repoints the first run's breadcrumb to the second tip, leaving the earlier deletion's commits advertised nowhere and exposed to garbage collection. A verify-after-write does not catch that — the ref resolves to exactly the tip just written. Reserve against an expected-nonexistent name instead, so the collision fails the write rather than the guarantee. This ordering is the point: creating the breadcrumb after `git branch -D` means an interruption or a failed ref write between the two steps leaves the commits dangling with no advertised way back, which is exactly the guarantee the skill sells.
...
- Never mutate origin: no pushes, no remote branch deletion, no PR operations. Remote access is read-only (`git fetch`, `git remote set-head origin --auto`, `gh` read queries) plus the optional default-branch pull — `set-head` writes only the local symbolic ref, never the remote.
...
- Every deletion path prints tip SHAs; non-Merged deletions get backup refs with documented restore instructions, and every such ref is created and verified **before** its branch is deleted. Creation claims an unused name rather than overwriting an existing one, so a repeat prune of the same branch cannot repoint an earlier run's breadcrumb; a taken name is disambiguated rather than refused, and only a name that still cannot be claimed drops its branch from the deletion set.
...
- Dry-run a repo whose default branch is neither `main` nor `master` and whose `origin/HEAD` is unset **and where neither the remote advertisement nor the API can resolve it** (no reachable remote, no `gh` auth): the skill must refuse to delete rather than heuristically selecting a branch. An unset `origin/HEAD` alone is not this case — with a reachable remote, `set-head --auto` resolves it and the run proceeds normally, which is the correct outcome, so the refusal path is reached only when every authoritative source fails. Dry-run the stale variant too — `origin/HEAD` still pointing at a branch the remote has since replaced as default: the refresh must correct it, and the branch that is *actually* default must never enter a deletion bucket. Cover the sub-case where the new default is still absent locally after step 1 — a checkout whose fetch refspec does not cover it (e.g. a `--single-branch` clone) or one whose fetch failed — so `set-head --auto` exits non-zero while the stale ref keeps resolving: the skill must reach the API rather than accept the stale answer. The precondition matters, because in an ordinary clone step 1's fetch supplies the ref and `set-head` then succeeds, which is the correct outcome and not this case.
