...
- **A declared allowlist for legitimate structural divergence**, checked in beside the suite as data, with a one-line reason per entry. **An entry pins the exact divergence, not the section holding it:** the skill, the heading, and the specific delta excused there — both mirrors' counts for a count divergence, the heading itself for a one-sided heading. A section-scoped entry is an exemption rather than an allowance, and it fails silently in the case that matters: where a section already diverges, one-sided bullets can be added to or deleted from it and the entry stays live, the stale-entry check never fires, and the "deleting a bullet from one mirror fails" criterion below is lost. Divergence not named there fails; a divergence that no longer matches its entry fails; an entry whose divergence has disappeared fails too, so the list cannot rot into a list of things that used to be true.
...
- Tasks 045, 046e, and 047a established the pattern for carrying one rule into every derived rendering — asserting the output-destination rule, deletion-guarding the destroy-boundary bullet across the skill mirrors, and carrying the exclude obligation onto the workflow surface. This task guards the shape of those renderings rather than restating any rule.
...
- **Measured starting point, as of this task's authoring on `main` at `e14ced7`.** Measured over ATX headings only — the `#` prefix at the start of a line, taken as level plus text, in document order, **with every line inside a fenced code block excluded**. On that measure the heading sequences are already identical for 10 of the 14 skills, and the four that differ are `address-review`, `address-reviews`, `address-tasks`, and `address-tasks-serialized`, each carrying exactly one heading the Claude mirror lacks: "Codex subagent execution" in the first three (at `###` in `address-review` and `##` in the other two, so the level is part of what is compared) and "How to spawn a subagent in Codex" at `###` in the fourth. `review-cycle`'s heading sequences are **identical**; a measurement that forgets to exclude fenced blocks counts the shell comments in its launch snippets as headings and reports a spurious four-heading divergence there, so state the exclusion in the suite and in any figure quoted from it.
- **The per-section count check has its own baseline**, and it is not the heading one. Measured the same way — list markers at column zero only, fenced blocks excluded, compared across sections whose heading appears in both mirrors — two sections diverge at `e14ced7`, both of them bullets the Codex mirror carries and the Claude mirror does not: `review-cycle` under "The peer step" (Claude 0, Codex 5 — the `claude` preflight classification ladder) and `resolve-open-questions` under "4. Serve each question — the core move" (Claude 0, Codex 2 — the structured-user-input-tool branch). Task 050 rewrites the peer launch inside that same "The peer step" section, so whichever of the two lands second re-derives this figure rather than assuming it survived.
- Both baselines are line-free, so they are quotable, but re-derive them rather than trusting them — the point of the suite is that nobody's recited count is authoritative.
- Inspect what each divergence actually is before writing a single allowlist entry, rather than entering it from the figures above. Each entry's reason should name the harness difference that produces it, so a reader can tell a legitimate divergence from one nobody has looked at yet — and no entry should exist for a divergence the suite does not report, since keeping one whose divergence is absent is itself a failure.
...
- The same holds inside an allowlisted section: deleting one of the excused one-sided bullets, or adding another beside them, fails the suite. An allowlisted divergence is a pinned delta, not an exempted region.
