...
- **Two rebase points**: before triage/fixing, and after fixes before the final review and push. In the prose skills the agent may judge a point unnecessary (e.g. base unchanged); in `wf-address-review` both points are unconditional (no-ops are cheap and deterministic).
  - **Amended by this task's own review (round 1, where the two reviewers split on it):** "unconditional" governs runs that reach the points — no point may be skipped because an agent judged it unnecessary — and does not require rebasing on a path that returns before the fix/review/publish pipeline. `wf-address-review` resolves the PR and gathers the threads in one step, so a run with nothing to address, and a run stopped at the branch-reconciliation gate, are already terminal by the line the first point would occupy — the first as a no-op, the second as a stop that hands the branch back; rebasing there would rewrite the maintainer's branch, diverge it from the PR head, and leave neither a verdict nor a push behind the rewrite. The peer reviewer's counter-point stands on its own merits — a branch with no feedback but an advanced base is not necessarily a rebase no-op — but what it argues against is the workflow's zero-item exit, which returns a no-op even where the local tip is already ahead of the PR head, a divergence from `address-review` step 3 that predates this task. Task 016a carries that; this task's text and its delivered workflow both state the exception rather than leaving it to a report.
...
- Mid-run base advancement caused by in-batch early merges (that integration lands with 033, which reuses this nugget).

## Context and references

- **Sequencing**: implement AFTER 012, which creates `plugins/dev-skills/workflows/` and the `wf-address-review.js` this task edits, and AFTER 014, which establishes where the rebase nugget is placed.
- 021 — the correctness guidance (stacked ordering hazards, hunk-vs-file conflict resolution, patch-id) this task turns from advice-on-request into default behavior; implement 021's text first or together, and reference rather than restate it.
...
- 014 — the `openQuestions[]` ferry the halted-conflict path reports through.
...
- Parent-map tracking must stay cheap to codify: one `gh pr view --json baseRefName,headRefName,headRepository,headRepositoryOwner,isCrossRepository` per entry plus recording each parent's pre- and post-rebase tips; resist reimplementing `rebase-stack`. Ask for the head repository fields, not the two branch names alone, because the parent/child match is the repo-qualified comparison 021 specifies. There is no base-repository field to ask for and none is needed: a PR's base always lives in the repository the PR itself is in, so the base side of the comparison is that repository and `baseRefName`, while the head side is `headRepositoryOwner`/`headRepository` (with `isCrossRepository` as the cheap short-circuit).
