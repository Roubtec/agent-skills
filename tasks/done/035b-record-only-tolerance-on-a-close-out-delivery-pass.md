...
Put them together and the promise does not hold on the close-out path. A non-confirming pass offers a close-out, is therefore told to run the delivery tier, and that full run surfaces the flake — which is the expected place for it, since tiered validation makes the delivery run the first full-suite run of most cycles. The policy then requires the diagnosis task to be committed, so the close-out range now carries a new task file. A task file states acceptance criteria, so the close-out check's `nonSemantic` question answers false and the close-out is forfeited for a normal reviewer round — whose reviewer is told the ROUND tier, not the delivery tier, because `cycleValidationTier` decides by position and a close-out-offering pass is not the confirmation pass — after which the confirmation pass owes the delivery tier again. So the cost is a second full-suite run, plus a reviewer-and-peer round and a confirmation pass, bought by a commit that adds a queue entry and a note. That is one suite run short of the three-runs cost the record-only tolerance spares on the confirmation-pass path it already covers, but it is the same tolerance denied for the same commit.
...
Choose option (a), a record-only suffix on the close-out range. The confirming-pass conjunct remains the safety boundary on the standalone record-only exit, so an intermediate pass that did not offer the bounded close-out cannot bypass a round while findings remain undisposed; the close-out's existing fixed-only, no-deviation-move, no-retirement, and diff-read gates instead authorize the narrow composition. The producer lists only the pre-suffix non-semantic fixes in `closeOutEdits`; ordinarily those fixes are the whole pass diff, with the exact final diagnosis-only commit required by the flake rule as the sole exception. Its `flakeRecord` accounts for that suffix and lets the producer offer the path, but does not self-certify or broaden it: any other semantic content forfeits the offer. The same read-only check identifies the exact final diagnosis-only commit, judges every preceding hunk by the existing non-semantic and completeness rules, and returns the suffix's exact range so the result can carry both records without treating the whole close-out range as record-only. The cycle's independent packet measurement resolves the committed HEAD and its actual parent, and the structural gate accepts the reported suffix only when both endpoints equal that measured parent-to-tip range; a valid-looking whole-pass range is not evidence of a one-commit suffix.
