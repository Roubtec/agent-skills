...
- **The pins**, in `scripts/test-address-review-reconcile.mjs`. Two reads currently match `aborted: "off-shoot does not carry the PR head"` as a closed literal: the `offStop` index, and the `gate` line lookup. Each feeds more than one check, and how many is deliberately not written down here — this task has already carried two different wrong counts, and any number written now drifts on the next check added nearby — so derive the set by perturbing the literal and reading which checks the run names rather than by matching a number. One consumer of `offStop` does not announce itself: the slice taken from `off.indexOf("normal push (", offStop)` loses its anchor silently, because `indexOf` given a `-1` start searches from 0, so that check keeps passing while the read it depends on has already failed. An appended-evidence form breaks both reads, so they move with the wording, and the suite's `EXPECTED_CHECKS` moves only if a check is genuinely added.
...
- Both reads of that literal still resolve against the new wording, and the negative control is run rather than reasoned about: remove the evidence instruction, run the suite, and account for every check the run names as failing. The criterion is that whole observed set — no count is fixed here for it to be matched against. A check that stays green is not evidence that its read survived, because of the silent `offStop` slice noted in Scope, so confirm each of the two reads resolves against the new wording directly as well.
