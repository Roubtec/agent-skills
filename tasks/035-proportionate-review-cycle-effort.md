...
- Task 019 — convergence heuristics landing in the same sections; implement this after 019 to avoid churny conflicts. Unlike 019, this task **does** amend one gate (item 3), knowingly.
...
- `tasks/done/014` — the extraction that makes the canonical block the single authoring point.
...
- Item 2's evidence requirement is the anti-evasion guard: "unrelated" is a demonstrated property (fails on base / untouched paths), never an assertion of convenience.
...
- The canonical block names both validation tiers, who selects the round tier, and the no-executable-change case that skips builds; the delivery tier is stated as mandatory before conclusion and publication and lists lint/typecheck/build/tests as-applicable.
- The flake policy is present with its evidence requirement, one-rerun bound, diagnosis-only task content, PR/summary note, non-blocking reviewer treatment, and the explicit duplicate-tasks-are-acceptable statement.
