...
- The guard resolves every open PR head to a readable tree via the base repo's `refs/pull/<N>/head` rather than assuming a local ref, enumerates open PRs without relying on a default result cap, and names any head it could not scan instead of silently omitting it.
