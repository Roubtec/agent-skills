...
- The matching rule distinguishes suffix families from true collisions: it compares full number+suffix, so `001`/`001a` never trip it, while `024`/`024` always does — whether the slugs differ or match across `tasks/` and `tasks/done/` — except where the scanning branch is itself relocating that one file.
