...
- Write the guard parallelism-ready for [033](033-vertical-task-pipelining.md), and **define the comparison set here, once** — 033 composes this guard rather than redefining it, so a second statement of the set there or anywhere else is a defect, not a summary. The set is the union of: the **base branch**; every **open PR head**; in a pipelined run, branches **delivered earlier in the same run** and anything **merged since the run started**; numbers **reserved** by a task that has cleared the guard but not yet finished delivering; and **currently-ready siblings** awaiting the guard. First claimant wins; the second claimant renumbers, and it is never the delivered or reserved side.
