...
1. **The record's field set lists "the permalink" as a separate field, which for a CI lane is the identity slot's own content.** The sentence beginning "**Every entry carries the same field set whatever its disposition**" resolves a lane's stable reference to its identity while explaining that the details URL "is the lane's permalink rather than its identity", and then goes on to require "the permalink" as the next field. For a lane those are the same URL wearing two hats, and an implementer reading the list literally records it twice or wonders which one is meant. Say once what each entry kind puts in each slot.
...
- The record's field set states, for each entry kind, exactly what goes in the stable-reference slot and what goes in the permalink slot, with no field a CI lane must fill twice.
