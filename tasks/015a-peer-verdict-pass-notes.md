...
- `wf-check` (or the runtime-wrapped `node --check`, per `plugins/dev-skills/workflows/README.md`) passes on any workflow file touched; a bare `node --check` on those sources cannot fail.
