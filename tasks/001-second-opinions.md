...
- The prompt must carry: the worktree path, the review scope (base branch or commit range), a **pre-computed diff artifact** for peers without shell access — before launching the guarded `claude -p` peer, the orchestrator writes `git log --oneline <base>..HEAD` plus `git diff <base>...HEAD` to a file **outside the worktree** (e.g. alongside the outfile) and passes its path, since without it the shell-less peer could only inspect the final tree, never the changed lines (the codex peer's read-only sandbox can run `git diff` itself, so there the artifact is optional) — the relevant context verbatim (task file, or review threads under verification), an instruction to read the actual files, an explicit "edit nothing", and the **output contract**: `VERDICT: PASS | ISSUES` followed by numbered findings, each tagged `blocking` or `minor` with `file:line` and a one-line rationale.
