...
- From Claude skills: `codex exec --sandbox read-only --cd <worktree> -o <outfile> "<prompt>"`. From Codex skills: `claude -p "<prompt>" > <outfile> 2>&1` with the working directory set to the worktree — `claude` has no `-o` flag (verified: only `--output-format`), so the redirect **is** the capture contract: every background peer invocation writes to its own per-invocation outfile, because concurrent invocations (e.g. an `address-reviews` batch) would otherwise interleave on the parent's stream and leave no stable artifact to parse after the reviewer barrier; `--output-format json` makes the captured artifact parseable.
