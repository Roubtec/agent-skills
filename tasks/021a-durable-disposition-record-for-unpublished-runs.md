...
- **Record SHAs as provenance, never as a replay gate.** This is the correctness requirement, not a detail. The maintainer's objection was that *"the branch may want a rebase before the final push"*, which invalidates any `final HEAD == branch tip` assertion while changing nothing about whether the work is still there. What replay must establish is that the reviewed work is still in the branch, and it must re-derive each `Fixed in <sha>` citation from the branch at replay time rather than replaying a stale SHA. A replay keyed on SHA equality is a failed implementation of this task however well it stores the record.
- **Patch-id is the first probe, not the gate.** With `F` the recorded final tip and `B` the branch tip, an empty `git rev-list --right-only --cherry-pick B...F` means every recorded commit is represented and the record replays as written — the same test PR #29 adopted for branch reconciliation. A non-empty result must **not** reject the record: a rebase that resolved a conflict, or that split or squashed a commit, mutates the resulting commit's patch-id while keeping the work, which `rebase-stack` documents as the patch-id cascade, and such a rebase is precisely this task's validation scenario. Fall through to the branch's current tree as the authority — the same move the merged-parent guard makes when it declines to conclude from patch-id — and judge per thread whether the fix its disposition claims is present in the branch as it now stands. A thread whose fix is genuinely gone loses its disposition and must not be replied to as fixed; a case the tree cannot settle goes to the maintainer with the record intact rather than being discarded.
...
- Replay accepts a record whose commits survived a rebase that had to resolve a conflict, and re-derives SHA citations. A design asserting `final HEAD == branch tip`, or one that rejects the record because the recorded commits' patch-ids no longer match, fails this criterion — a pre-push rebase is the expected case rather than an edge case.
...
- Both mirrors of each skill stay in parity apart from their harness-specific lines. A change landing in one flavor and not the other, in the same change, fails.

## Validation

- Walk the PR #29 scenario: a descendant reviewed and blocked behind a parent at the round cap, session ended, new session replays from the comment alone — without the original context, and after an intervening rebase of the descendant branch that had to resolve a conflict, so the recorded commits' patch-ids no longer match.

## Review plan

Reviewer confirms the record is SHA-independent for validity and survives a conflict-altering rebase rather than being gated on patch-id equality, that every stated `no-push` contract location agrees after the change, and that the record format is defined in one place rather than copied into each caller.
