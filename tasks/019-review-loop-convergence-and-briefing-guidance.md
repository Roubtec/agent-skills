...
2. **Same-class findings ⇒ structural defect.** When N consecutive rounds land findings of the same class in the same section, stop patching instances and ask whether the structure is the defect. Name the two observed triggers: a closed enumeration standing in for an open set (replace with an exclusion rule), and a spec keeping multiple options open so every criterion must hold under all of them (lock one option). (A Scribz task file drew six rounds this way; both restructurings ended their finding classes outright.)
...
6. **Diff bases after amend/rebase.** When a fix commit is amended between rounds, hand the re-reviewer the correct comparison base (the fix commit's parent or the prior-round SHA), never a bare "diff `HEAD~1`" (an amend makes that the whole fix set and produced a false "unrelated changes" blocking finding); alternatively prefer fixup commits squashed at the end. After any rebase, delegate ranges relative to stable refs (`origin/main..HEAD`), never absolute SHAs captured pre-rebase.
...
- `plugins/dev-skills/skills/address-review/SKILL.md` (primary)
- `plugins/dev-skills/skills/address-reviews/SKILL.md`, codex-side mirrors (pointers/mirrors)
