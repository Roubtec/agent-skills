...
3. **Check-status polling.** In `gh pr view --json statusCheckRollup`, in-progress checks have `conclusion: ""` (empty string, not null), so jq `//` fallbacks like `.conclusion // .status` never fall through and a poll loop exits early reporting checks settled. Branch on `__typename` first — the rollup is a union of `CheckRun` and `StatusContext`, and only `CheckRun` has `status`/`conclusion`; a legacy commit status is a `StatusContext` carrying `state`. Treat a `CheckRun` as settled when `.status == "COMPLETED"` (verdict from `.conclusion`), and a `StatusContext` as settled when `.state` is one of `SUCCESS`, `FAILURE`, `ERROR` (that `.state` is the verdict). Test each terminal set positively: `EXPECTED` is a required context that has not reported, so excluding `PENDING` alone settles it early. Bound the loop with an overall timeout and report what was still unsettled, since a configured-but-never-posted context stays `EXPECTED` indefinitely.
