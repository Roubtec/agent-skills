...
3. **Check-status polling.** In `gh pr view --json statusCheckRollup`, branch on `__typename`: a `CheckRun` is settled when `.status == "COMPLETED"` (verdict `.conclusion`), a `StatusContext` when `.state` is `SUCCESS`, `FAILURE`, or `ERROR` (verdict `.state`). Test both terminal sets positively — an in-progress `CheckRun` reports `conclusion: ""`, not null, so jq `//` fallbacks never fire, and a `StatusContext` carries no `status` at all, so excluding `PENDING` alone wrongly settles `EXPECTED` (required, not yet reported). Bound the loop with an overall timeout and name whatever is still unsettled.
