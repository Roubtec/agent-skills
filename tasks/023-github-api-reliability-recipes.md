...
3. **Check-status polling.** In `gh pr view --json statusCheckRollup`, in-progress checks have `conclusion: ""` (empty string, not null), so jq `//` fallbacks like `.conclusion // .status` never fall through and a poll loop exits early reporting checks settled. Poll on `.status != "COMPLETED"`; read `.conclusion` only once `.status == "COMPLETED"`. **Branch on `__typename` first:** the rollup is a union, and a legacy commit status arrives as a `StatusContext`, which carries `state` (`PENDING`/`SUCCESS`/`FAILURE`/`ERROR`/`EXPECTED`) and has no `status` or `conclusion` at all — those belong to `CheckRun`. Applied blindly, `.status != "COMPLETED"` is therefore true forever on a `StatusContext` and the poll never settles, the mirror image of the early-exit bug. Treat a `CheckRun` as done when `.status == "COMPLETED"` (verdict from `.conclusion`), and a `StatusContext` as done only when `.state` is one of `SUCCESS`, `FAILURE`, or `ERROR` (the verdict is that `.state`). Test the terminal set positively rather than excluding `PENDING` alone: `EXPECTED` means a required context that has not reported yet, so a `.state != "PENDING"` test settles it early and reintroduces the very bug this recipe exists to prevent, one union member over.
