...
The first half is contradicted by the counterpart's own measurement, and that measurement has since become a shipped decision rather than a recommendation. Powbox task 061, in the `Roubtec/powbox` repository, records under "Placement and scope decisions" that `TMPDIR` is unset in a current agent container and `/tmp` is the disk-backed overlay filesystem, so clones already land container-local and outside `/workspace` — which is the placement powbox would have set the variable to express. That task is now merged (commit `13d3b83`), and its `docs/architecture.md` entry for the baked `dc-enter`/`dc-remove` pair states outright that powbox "deliberately sets **neither** `DC_ROOT` nor `DC_AGENT`, because both helpers' defaults already land where powbox would point them". So the replacement fact is available and durable: the header cannot keep asserting powbox *will* set the variable, and the reason is no longer that nobody has decided — it is that powbox decided not to.
...
**Fold this into task 046b.** Declined at the time: 046b's branch was then scoped to `README.md` and its own task file while three sibling tasks were editing the skill, workflow, and `scripts/` trees concurrently, and touching a shipped helper there was an avoidable collision. 046b's scope later widened to those same trees once its powbox gate opened, but the decision stands on its second ground rather than being revisited on the first: the split keeps the two changes reviewable for what they are — 046b removes a claim from documentation, this removes one from an executable's contract text.

**Set `DC_ROOT` in the powbox image so the prediction comes true instead.** Not this repository's call, and powbox has decided against it: task 061 measured that the default placement is already correct there, so setting the variable would be configuration for its own sake, and its merge recorded that decision in `docs/architecture.md`. If it is ever revisited, this header should still describe the interface rather than one consumer's use of it.
