...
- **Worktree lifecycle**: branch resolution and attach follow `address-reviews`' "Resolving and checking out each entry" rules wholesale — local-first (a local ref wins over `origin`), `origin` head only when no local branch exists, and fork PRs via the detached-worktree + `gh pr checkout` case (reference, do not restate) — with the one exception under Implementation notes below, where a local ref bearing the target's name has already failed the PR identity check and local-first is suspended for it; that exception must land the verified PR head on a **durable local branch that is not the rejected ref**, under a disambiguated name the run reports. State the requirement rather than one blessed command, because the two obvious shortcuts fail in opposite directions and the pair of failures *is* the requirement. Attaching by branch name lands on the rejected ref and succeeds silently — `wt-enter` verifies the branch is checked out, which it is, so unrelated history passes without complaint, and a bare `gh pr checkout` derives the same name from `headRefName` and gets there too, aborting loudly on unrelated histories only by luck of the merge. Checking out detached dodges the ref but leaves no branch, and the rest of this bullet needs one: `wt-remove` guards only uncommitted changes, so a `no-push` finish reclaims the worktree and leaves the fix commits reachable from nothing, and a halted run's worktree cannot be re-entered by slug. Any mechanic satisfying both constraints will do — naming a fresh branch at checkout time is the cheapest. Note that the fork recipe referenced above fails the first constraint — its bare checkout selects the rejected ref, so it does leave a durable branch, just the wrong one — and it is hardened where it lives rather than worked around here, under the same two constraints, since satisfying only the second is how a detached fix-up would orphan a fork PR's commits in turn; after publication (or a `no-push` finish) the worktree is removed via `wt-remove` — the branch ref persists in the shared `.git` — and a worktree kept alive by a halt (blocker, unresolved conflict) is reported with its path.
...
- **Fork-recipe hardening** in `address-reviews`: the attach recipe this task inherits by reference pairs `git worktree add --detach` with a bare `gh pr checkout <N>`, and the detach does not stop gh selecting a same-named local branch. One line, fixed where the recipe lives so every caller benefits rather than each working around it.
...
- On that exception path the work lands on a durable local branch under a reported name — so its commits survive the worktree being reclaimed, and a text prescribing a detached checkout fails this criterion however correctly it avoids the rejected ref.
- The `address-reviews` attach recipe lands the PR head on a durable local branch that a same-named local ref cannot capture — the same two constraints as the exception above, stated as properties rather than as a command pairing that must disappear, since a detached fix-up satisfies the latter while orphaning a fork PR's commits at the next worktree reclaim.
