...
- **Worktree lifecycle**: branch resolution and attach follow `address-reviews`' "Resolving and checking out each entry" rules wholesale — local-first (a local ref wins over `origin`), `origin` head only when no local branch exists, and fork PRs via the detached-worktree + `gh pr checkout` case (reference, do not restate) — with the one exception under Implementation notes below, where a local ref bearing the target's name has already failed the PR identity check and local-first is suspended for it; attaching there puts the PR head on a **fresh, disambiguated local branch** — `gh pr checkout <N> -b <slug-qualified-name>` — rather than borrowing the fork recipe wholesale, and the naming is the whole exception. Bare `gh pr checkout` derives its branch name from `headRefName`, so with the rejected same-named ref present it checks *that* ref out and fast-forwards it: verified, it lands on the unrelated tip and aborts with `refusing to merge unrelated histories`, and creating the worktree with `--detach` first does not prevent it. A by-name attach such as `wt-enter <slug> <branch>` is worse, landing on the same rejected ref and exiting 0 without complaint. `gh pr checkout <N> --detach` avoids both but costs the branch the rest of this bullet depends on — `wt-remove` guards only uncommitted changes, so reclaiming a detached worktree after a `no-push` finish leaves the fix commits reachable from nothing; publication has no ref to push or lease against; and a halted run's worktree cannot be re-entered by slug. A fresh branch keeps all three, which is why it is the prescription and not the aside. The referenced fork recipe carries the same bare-checkout trap and should be hardened where it lives rather than worked around here; after publication (or a `no-push` finish) the worktree is removed via `wt-remove` — the branch ref persists in the shared `.git` — and a worktree kept alive by a halt (blocker, unresolved conflict) is reported with its path.
