...
- The `address-reviews` attach recipe verifies that what it checked out carries the PR head, so a same-named local ref cannot capture it silently, and on the success path it still lands that head on a durable local branch — a detached fix-up fails this criterion however correctly it avoids the rejected ref, since it orphans a fork PR's commits at the next worktree reclaim.
