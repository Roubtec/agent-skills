...
- **Fork-recipe hardening** in `address-reviews`: the attach recipe this task inherits by reference pairs `git worktree add --detach` with a bare `gh pr checkout <N>`, and the detach does not stop gh selecting a same-named local branch — so the recipe can land on unrelated history and report success. Fixed where the recipe lives so every caller benefits rather than each working around it: after the checkout, verify that what it landed on carries the verified PR head, and treat a mismatch as the same stop-and-ask above. Hardening it by detaching instead is not available, since a fork PR's commits would then be reachable from nothing once `wt-remove` reclaims the worktree — trading a wrong branch for no branch.
...
- Mode detection starts with one comparison (`git rev-parse --abbrev-ref HEAD` vs the resolved target branch) but a name match alone is not enough: mode 1 additionally requires the skill's existing PR-vs-branch sanity check to pass (shared recent history with the PR head; for fork PRs, the current branch's resolved push remote/ref matching the PR head repo/ref, as the publish preflight already computes) — a same-named but unrelated local branch falls through to mode 2 — but never *onto that branch*: the local-first attach rule is suspended for a ref that just failed the identity check, and the run stops there with the mismatch surfaced rather than attaching anything at all. Inheriting local-first unqualified would hand the fixer the very ref the check rejected, landing review changes on unrelated history and leaving the publication preflight, many commits later, as the first thing to notice; auto-picking a substitute name instead only relocates the problem, since the new branch has no verified push target and the stale local ref survives to collide on the next run. Reusing a name like `minor-fixes` or `batch-wrap-up` across unrelated work is normal maintainer practice, so this collision is expected rather than exotic, and the maintainer's answer beats any heuristic the run could apply. Detached HEAD counts as "any other branch" — there is nothing to advance under the user.
...
- The `address-reviews` attach recipe verifies that what it checked out carries the PR head, so a same-named local ref cannot capture it silently, and it still lands that head on a durable local branch — a detached fix-up fails this criterion however correctly it avoids the rejected ref, since it orphans a fork PR's commits at the next worktree reclaim.
