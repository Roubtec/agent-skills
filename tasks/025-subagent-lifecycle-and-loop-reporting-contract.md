...
- Item 6's sites are wherever the serialization rule is stated today as a property of turns, which at PR #58 is `address-tasks-serialized` (its Architecture rule and both spawn instructions), `address-review`'s shared-working-tree note, `address-tasks`'s per-worktree restatement of the inherited rule, and `review-cycle`, in both trees. `review-cycle` already names the on-disk invariant, so there the addition is only the asynchronous-spawn note and the demotion of the turn rule to a proxy. Re-derive the set before editing rather than trusting this list: grep both trees for "same turn", "parallel tool block", and "foreground".
...
- Item 6 is a rewrite of text that already exists — the shared-working-tree rule and each spawn instruction that says "foreground" or "same turn" — not a sixth paragraph added beside them. Every one of those sites is edited or it has not been delivered: a single new statement of the invariant elsewhere leaves the turn-shaped ones still reading as the rule.
