...
6. **Serialize on committed state, not on turn structure.** The rule that a reviewer never starts before its own implementer is currently expressed as a property of the orchestrator's turns — the implementer "is launched in the **foreground** (not background)", and two checkout-dependent agents must never sit "in the same turn or parallel tool block" (e.g. `plugins/dev-skills/skills/address-tasks-serialized/SKILL.md:25`, `plugins/dev-skills/skills/address-tasks-serialized/SKILL.md:65`, and `plugins/dev-skills/skills/address-tasks-serialized/SKILL.md:122`, all as of PR #58 and none of them the whole set — see Target files below for how to derive it — and inherited by reference into `plugins/dev-skills/skills/address-tasks/SKILL.md`; the codex mirror states the same rule in its own harness-specific wording, which never says "foreground"). In the Claude Code harness there is no foreground mode to select: an `Agent` call returns immediately with a background task id and the result arrives as a later notification, so "wait for its result" is a turn boundary rather than an inline wait. The invariant that actually matters is never named: **a reviewer may not start until its implementer's commits are on disk.** State it that way, note that a harness may run spawns asynchronously and that the orchestrator's obligation is to wait for the completion notification, and keep the same-turn rule only as the proxy it is — sufficient where spawns are synchronous, and neither sufficient nor necessary where they are not. A batch on 2026-08-08 ran three implementers from one tool block and enforced the real constraint by phase ordering plus worktree separation; nothing broke, but the shipped explanation of *why* it is safe was wrong.

Out of scope: peer-launch mechanics (task 015), scratch hygiene (task 017), and the destroy boundary's addressing form and diagnosis-discipline instrument (task 017b).
...
- Item 6's sites are wherever a reviewer is serialized behind its implementer today: at PR #58, the five skills `address-tasks-serialized`, `address-review`, `address-reviews`, `address-tasks`, and `review-cycle`, in both trees — the ten files `grep -rlE 'checkout-dependent|worktree must' plugins/ codex/` returns, which run at PR #58 matched exactly those ten and nothing else. Re-run it to pick up a file added since, but treat it as a locator for **files**, not lines, and read each matched file end to end: every file restates the rule several times in wording that differs per mirror, so no term grep reaches every line. The codex mirrors say "natural-language turn" with "tool block" or "tool-call batch" and never say "foreground" anywhere; `address-tasks` says "subagents sharing one worktree must be serialized" and `address-reviews` "agents sharing *one* worktree must run one-at-a-time" — both name ordering rather than committed state, so both need the rewrite; `review-cycle` and `address-review` say "one checkout-dependent agent at a time". `address-tasks-serialized` carries the most restatements: its Architecture rule, its Orchestrator Responsibilities step, the Implementer and Reviewer sections' spawn instructions, the Execution Model note, both Per-Task Workflow spawn steps, and the Feedback Loop re-review step, plus a "How to spawn a subagent in Codex" section that exists only in the codex mirror. `review-cycle` already names the on-disk invariant, so there the addition is only the asynchronous-spawn note and the demotion of the turn rule to a proxy.

## Implementation notes

- Items 1–2 belong verbatim in the subagent prompt templates (they bind the subagent); items 3, 5, and 6 bind the orchestrator; item 4 needs both sides (implementer reporting shape, reviewer recommendation, orchestrator surfacing).
- Item 6 is a rewrite of text that already exists — the shared-working-tree rule and every restatement of it in each of the ten files, whatever wording that mirror uses — not a sixth paragraph added beside them. Every one of those sites is edited or it has not been delivered: a single new statement of the invariant elsewhere leaves the turn-shaped ones still reading as the rule.
...
- Every site that today serializes a reviewer behind its implementer states the committed-state invariant as the rule — a reviewer is not spawned until its implementer's commits are on disk — says that a harness may run spawns asynchronously so the orchestrator's obligation is to wait for the completion notification, and keeps any "foreground" or same-turn wording only as the proxy it is. No site is left stating the turn property alone.

## Validation

- Replay each of the four incidents against the new text: the contract now names the violation at the moment it occurred.
- Replay item 6 against the 2026-08-08 batch that launched three implementers from one tool block: the new text must read that run as compliant, because each reviewer waited for its own implementer's commits, while still failing a reviewer spawned before its implementer's packet returned. Text that only forbids the shared tool block gets the first half wrong, and text that only forbids a concurrent reviewer gets the second half wrong.
