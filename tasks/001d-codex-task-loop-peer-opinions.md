...
- Peer invocation from this tree: `claude -p "<prompt>"` executed with the working directory set to the task's worktree (optionally `--output-format json`; effort per the umbrella §2 quality floor — request `--effort high` when the peer's configured default is not known to be high/xhigh; the peer is examination-only, so pass the explicit read-only guard from umbrella §2 — e.g. `--tools "Read,Glob,Grep" --disallowedTools "mcp__*"` — and never permission-bypass flags). The prompt carries the umbrella contract: worktree path, base branch/commit range, task context verbatim, "edit nothing", `VERDICT` + numbered `blocking`/`minor` findings with `file:line`.
