...
- The "decision that writes code" flow: launch `claude -p "<prompt>"` (cwd = the working tree, umbrella prompt/output contract, ~10-minute timeout, one retry) in the background alongside the fresh-eyes reviewer, scoped to the decision's commit range.
