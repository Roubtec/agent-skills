...
- While editing the commit step: have it commit only this change. The step stages `.powbox.yml` and `.gitignore` wholesale, so unrelated uncommitted edits those files already carry would be silently absorbed — the same pattern PR #34's review fixed in `declare-shadows` step 8.
