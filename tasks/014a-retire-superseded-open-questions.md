...
- Parse-check every touched workflow script with `wf-check`, or with the runtime-wrapped `node --check` documented in `plugins/dev-skills/workflows/README.md`'s Validation section — a bare `node --check` on these sources cannot fail, so it is not the gate.
