...
- Add the placement rule (queued `tasks/` by default; `tasks/deferred/` only on named criteria) where task files get written: `address-review` step 5, `write-tasks`, and `resolve-open-questions`' task-hygiene section (hoisted by task 007).
...
5. **`write-tasks`**: add a short "Placement" note — when writing tasks in a session, settle queued-vs-deferred (and numbering) with the user per the note-2 criteria; default `tasks/`.
...
- The placement rule with the lean-toward-`tasks/` default and the named deferred criteria appears at all three writing sites (address-review step 5, write-tasks, resolve-open-questions task hygiene), phrased once per file, not duplicated within a file.
...
Reviewer greps for leftover `deferred-to-task` and for "defer" used on queued work, reads the three placement-rule sites for consistency of criteria and default, confirms frontmatter stability and flavor parity, and checks the `tasks/AGENTS.md` reword matches the maintainer's model stated in this task.
