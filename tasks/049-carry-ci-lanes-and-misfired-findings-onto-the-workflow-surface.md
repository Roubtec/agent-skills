...
- **Disposition record and replay** carry both kinds: a CI item and a misfired finding both need an entry shape the record can replay, keyed by check name (the run URL is per head, so it is the lane's permalink, and replay matches a recorded lane to the new head's rollup by name), or comment permalink plus finding ordinal, in place of a thread id, and the spend/supersede rules apply unchanged.
