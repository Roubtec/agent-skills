...
- Add a **fallback** paragraph for environments without the helper (`command -v peer-review-run` fails): the hardened manual pattern — write the prompt via quoted heredoc (`<<'PROMPT'`), launch with `nohup … &` with stdin redirected (never left on the caller's terminal) and a unique per-attempt output path, check liveness by a unique token in the output path (never the prompt text), kill by PID captured at launch (never `pkill -f`), treat a non-empty result artifact containing a `VERDICT:` line as authoritative, and never run the peer as a capped foreground call. **Two of those rules are provider-specific and must be written that way, not mirrored verbatim** — the codex wording copied symmetrically would leave the `codex/dev-skills/` mirrors, whose peer is Claude, describing a launch their provider does not support. *Result capture:* codex writes via `-o <path>`, but `claude` has no `-o` flag, so a Claude peer captures redirected stdout with stderr kept in a separate file (`claude -p … > <stdout-file> 2> <stderr-file>`, never `2>&1`). *Stdin:* codex takes its prompt as an argument and so closes stdin with `< /dev/null`, whereas `claude -p` is fed its prompt file **on** stdin (`< "$prompt_file"`) — which is also what keeps the prompt literal rather than shell-interpolated. Both per the established contract in `tasks/done/001d-codex-task-loop-peer-opinions.md` and the delivered mirrors (`codex/dev-skills/skills/address-tasks/SKILL.md:186`).
...
- The fallback paragraph exists once (or once per skill where structure demands) with the hardening rules above, and its provider-specific rules are stated per provider rather than mirrored from the codex form: result capture (codex `-o`, Claude redirected stdout) and stdin handling (codex `< /dev/null`, Claude `< "$prompt_file"`).
