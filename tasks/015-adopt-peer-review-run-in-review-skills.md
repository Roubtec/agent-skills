...
- The fallback paragraph exists once (or once per skill where structure demands), states the condition that triggers it (`command -v peer-review-run` fails) and the hardening rules above, and its provider-specific rules are stated per provider rather than mirrored from the codex form: result capture (codex `-o`, Claude redirected stdout), stdin handling (codex `< /dev/null`, Claude `< "$prompt_file"`), and liveness (codex `pgrep -f` on the unique output path, Claude `kill -0` on the launch-captured PID or process group, never a path token the redirection keeps out of argv), with `resolve-open-questions`'s codex mirror moved onto the stdin form alongside the other four.
