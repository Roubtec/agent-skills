...
- Add a **fallback** paragraph for environments without the helper (`command -v peer-review-run` fails): the hardened manual pattern — write the prompt via quoted heredoc (`<<'PROMPT'`), launch with `nohup … &` with stdin redirected (never left on the caller's terminal) and a unique per-attempt output path, check liveness by a unique token in the output path (never the prompt text), kill by PID captured at launch (never `pkill -f`), treat a non-empty result artifact containing a `VERDICT:` line as authoritative, and never run the peer as a capped foreground call. **Two of those rules are provider-specific and must be written that way, not mirrored verbatim** into the `codex/dev-skills/` mirrors, whose peer is Claude. *Result capture* is a hard capability difference: codex writes via `-o <path>`, but `claude` has no `-o` flag at all, so a Claude peer captures redirected stdout with stderr kept in a separate file (`claude -p … > <stdout-file> 2> <stderr-file>`, never `2>&1`) — the contract already delivered in `tasks/done/001d-codex-task-loop-peer-opinions.md:21`. *Stdin* is a house-style choice rather than a capability limit, and the delivered mirrors are currently split, so state it rather than leaving it to be copied: codex takes its prompt as an argument and closes stdin with `< /dev/null`, while `claude -p` accepts the prompt either positionally or on stdin and the mirrors should standardize on the stdin form (`< "$prompt_file"`), which keeps the prompt literal rather than shell-interpolated. Four mirrors already do this (`codex/dev-skills/skills/address-tasks/SKILL.md:186`, `address-tasks-serialized/SKILL.md:258`, `address-review/SKILL.md:214`, `address-reviews/SKILL.md:196`); `resolve-open-questions/SKILL.md:206` still uses the positional form and should be brought in line as part of this task. Closing stdin outright is the codex-side reading of the "stdin redirected, never left on the caller's terminal" rule above, and must not be mirrored as such.
