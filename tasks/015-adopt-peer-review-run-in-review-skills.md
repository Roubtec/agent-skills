...
- The fallback paragraph exists once (or once per skill where structure demands) with the hardening rules above, and its provider-specific rules are stated per provider rather than mirrored from the codex form: result capture (codex `-o`, Claude redirected stdout) and stdin handling (codex `< /dev/null`, Claude `< "$prompt_file"`), with `resolve-open-questions`'s codex mirror moved onto the stdin form alongside the other four.
