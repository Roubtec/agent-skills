...
- **Incremental collision guard**: when a task's cycle passes, run it through a single serialized guard step (first-ready-wins) before delivery. This composes the same-number guard of [027](027-task-number-collision-guard-across-branches.md), which owns the comparison set and defines it in one place, pipelined members included. Do **not** enumerate that set here as the set, not even parenthetically: every partial restatement so far has silently dropped a member. Naming individual members while arguing about a specific scenario is fine — what is forbidden is a second list presenting itself as the membership rule. The second claimant of a number always renumbers, and it is always the undelivered side, which removes the "least disruptive side" judgment for the common case.
