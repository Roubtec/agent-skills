...
- **Incremental collision guard**: when a task's cycle passes, run it through a single serialized guard step (first-ready-wins) before delivery. This composes the same-number guard of [027](027-task-number-collision-guard-across-branches.md), which owns the comparison set and defines it in one place — including the pipelined members this task introduces (branches delivered earlier in the run, numbers reserved mid-delivery, currently-ready siblings). Do **not** restate that set here: every partial restatement so far has silently dropped a member. The second claimant of a number always renumbers, and it is always the undelivered side, which removes the "least disruptive side" judgment for the common case.
