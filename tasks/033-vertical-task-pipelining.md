...
- **Incremental collision guard**: when a task's cycle passes, run it through a single serialized guard step (first-ready-wins) before delivery. This composes the same-number guard of [027](027-task-number-collision-guard-across-branches.md), which owns the comparison set and defines it in one place, pipelined members included. Do **not** restate that set here, not even parenthetically: every partial restatement so far has silently dropped a member. The second claimant of a number always renumbers, and it is always the undelivered side, which removes the "least disruptive side" judgment for the common case.
...
- Batch summary correctly reports mixed terminal states including merged-during-run, and no batch ends holding an orphaned pushed branch: PR creation is retried or the branch deleted, and any that survives both is named in the summary with the task number it still holds.
...
Reviewer checks the guard's serialization cannot deadlock with held branches, that first-ready-wins never renames a delivered branch, that a number reserved by a task still mid-delivery is visible to the next task entering the guard and is not released while a pushed branch still holds it, that the comparison set is defined only in 027 and referenced rather than restated here, that a branch pushed without its PR cannot survive the batch unreported, and that removing the wave barriers did not remove the dependency gate itself.
