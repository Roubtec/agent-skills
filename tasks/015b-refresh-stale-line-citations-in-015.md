...
Locate the construct the prose names, then read its line number off the file — never shift an existing number by a computed offset. The offsets differ per file, several anchors crossed into a different file entirely, and the two causes above overlap in the codex mirrors, so an offset that looks right for one citation is wrong for the next.
