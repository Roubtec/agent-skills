...
- **A new stand-alone skill module `resolve-tasks`** in `plugins/dev-skills/skills/`, invocable directly by a user and consumable by other skills. Argument: a deduplicable mixed list of task numbers, task file paths, and globs. Behavior: resolve the repository's task folder, normalize each number per the numbering convention that folder actually uses — here the documented three digits plus optional lowercase letter, accepting unpadded input like `27` → `027`; a consumer repo on a different documented or de-facto scheme (`write-tasks`' fallback styles produce names like `A-01-…`) resolves by its own convention, inferred from the folder's files, rather than being forced through this one — search the entire subtree recursively including `done/`, `deferred/`, and any nested folders, and return (a) the deduplicated hard list of well-formed task spec paths, each tagged with the raw input or inputs — number, path, or glob — that selected it, so a consumer can apply an input-form policy without re-resolving anything, (b) a classification per resolved full number — `active`, `done`, `deferred`, or `ambiguous` (multiple files sharing one full number) — never per raw input: a glob is not classified as a unit; each file it matches classifies individually under the full number it carries, and (c) a `not-found` diagnostic per raw input — number, path, or glob — that resolved to no task file, kept apart from the number classifications because an unmatched input has no matched file to classify. Read-only: it resolves and reports, it never moves, renumbers, or edits task files.
- **Adoption in the three consumers:** each preflight runs `resolve-tasks` in a subagent with its own context window (the scavenging must not pollute the orchestrator's context) and receives back the hard path list plus the classifications and any `not-found` diagnostics. Consumer policy on anything short of a clean all-`active` resolution — a policy for *number* inputs: a path or glob the caller spelled out is an explicit selection, so the files it names or matches execute exactly as they do today whatever their classification, which travels along as report context only — the resolver's per-path provenance tags are what the consumer keys this split off, and a file selected both ways (a number and an explicit path or glob) follows the explicit selection; the `not-found` diagnostic is the one thing every input form gains, since an input that matched nothing was never going to run and surfacing it changes no execution. Interactively, surface a continue-prompt showing the resolved mapping — proceeding needs explicit confirmation when any number-resolved full number classified `done`, `deferred`, or `ambiguous` or any input came back `not-found`, and a bare-numbers invocation shows its resolved list before execution so caller and agent agree on what will run. For an `ambiguous` number, confirmation means a per-number choice — the caller selects which file the number means or excludes it — never a blanket continue that would run every matched file. Hands-off: proceed with the number-resolved unambiguous `active` set plus whatever explicit paths and globs selected, and document every exclusion — non-`active` number classifications and `not-found` inputs alike; never guess an ambiguous number.
- **The workflow rendering of `address-tasks`:** the `resolvePrompt` plan-stage subagent in `wf-address-tasks.js` is already the own-context-window scavenger this adoption asks for, and its argument contract ("a glob or file list") must not drift from the skill once numbers become first-class there. Extend that prompt to accept task numbers per the `resolve-tasks` contract, applying the hands-off policy only — exclude and document non-`active` number classifications and `not-found` inputs in the structured plan, with explicit paths and globs still executing what they name or match; the workflow has no interactive continue-prompt, so it never confirms, and it never guesses.
- **Consumer-specific readings:** for `reap-tasks`, a number resolving only into `done/` means already reaped — report it, do not re-verify it; for the implement skills, a number classified `done` means already delivered (prompt/skip) and one classified `deferred` means deliberately unscheduled, so implementing it needs explicit confirmation rather than silent inclusion.
...
- `README.md`'s Layout overview — its dev-skills enumeration names the shared building blocks, and a resolver three consumer skills route their arguments through belongs beside them.
...
- `resolve-tasks` exists, handles a mixed numbers/paths/globs argument resolved per the repository's own documented-or-inferred numbering convention, returns the deduplicated path list with each path's selecting input(s), the four-way per-full-number classification (globs classifying per matched file), and the per-input `not-found` diagnostic for inputs matching no task file, and states its read-only contract and the PR-numbers non-applicability note.
- All three consumers document the subagent-preflight invocation, the interactive confirmation on any non-`active` number classification or `not-found` input (including the bare-numbers echo of the resolved list, and a per-number select-or-exclude for each `ambiguous` number), and the hands-off exclude-and-document degradation — both policies binding number inputs only, with explicitly supplied paths and globs executing as given; the `wf-address-tasks.js` resolve stage accepts numbers under the same contract with the hands-off policy and reports exclusions in its plan.
- A numbers-only invocation of each consumer is now a documented, first-class form; existing path/glob invocations are unchanged in what they execute, gaining only the `not-found` surfacing for an input that matched nothing.
- Codex mirrors exist and match; `README.md`'s Layout overview mentions the new skill.
