...
- **Adoption in the three consumers:** each preflight runs `resolve-tasks` in a subagent with its own context window (the scavenging must not pollute the orchestrator's context) and receives back the hard path list plus the classifications and any `not-found` diagnostics. Consumer policy on anything short of a clean all-`active` resolution: interactively, surface a continue-prompt showing the resolved mapping — proceeding needs explicit confirmation when any full number classified `done`, `deferred`, or `ambiguous` or any input came back `not-found`, and a bare-numbers invocation shows its resolved list before execution so caller and agent agree on what will run. For an `ambiguous` number, confirmation means a per-number choice — the caller selects which file the number means or excludes it — never a blanket continue that would run every matched file. Hands-off: proceed with the unambiguous `active` set only and document every exclusion — non-`active` classifications and `not-found` inputs alike; never guess an ambiguous number.
...
- All three consumers document the subagent-preflight invocation, the interactive confirmation on any non-`active` classification or `not-found` input (including the bare-numbers echo of the resolved list, and a per-number select-or-exclude for each `ambiguous` number), and the hands-off exclude-and-document degradation; the `wf-address-tasks.js` resolve stage accepts numbers under the same contract with the hands-off policy and reports exclusions in its plan.
