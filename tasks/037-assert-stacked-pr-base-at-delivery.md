...
- **Record the practice where the repo documents PR practice:** how we open and merge PRs lives in `README.md`'s Contributing section, so the honest-diff base rule and the assert-then-repair policy get a sentence or two there as well — README records this repository's practice, while the skills keep authoring the intent where their base rules live, because they run in consumer repos that do not carry this README.
...
- Each of the three delivery points performs the post-create base assertion with the repair-then-report escalation, addressing the PR that `gh pr create` returned explicitly rather than relying on the current branch to select it, and recovering the PR by the recorded-head lookup in the base repository — with the head-owner match, since `--head` cannot name an owner — when creation fails before printing a URL (retrying creation only when the lookup finds nothing, and failing distinctly when delivery ends with neither a captured URL nor a lookup match). The honest-diff rationale is authored only where the base rules live and in README's Contributing section — delivery points carry the assertion mechanics, not a restated rationale.
