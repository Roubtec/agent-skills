...
Settled is not green, so test the verdicts positively in the same spirit: merge only once every `CheckRun` conclusion is `SUCCESS`, `NEUTRAL`, or `SKIPPED` and every `StatusContext` state is `SUCCESS`, and stop on anything else — `FAILURE`, `TIMED_OUT`, `CANCELLED`, `ACTION_REQUIRED`, `STARTUP_FAILURE` and `STALE` all report as settled, so a poll that only waits for settlement happily lands a known-red head. Nothing downstream re-checks this for you: `--match-head-commit` pins the commit, not its verdicts, and branch protection blocks only the checks it has actually been configured to require.
