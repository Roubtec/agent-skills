...
A parent that *cannot* publish — blocked at the 12-round cap, holding a blocker, or awaiting a maintainer decision — blocks only itself. This is the exception, and it covers a parent that became blocked **after** its dependent's wave had already launched (an ordinary fix-up round that ran out, or a publication-boundary rerun that failed), which is the only way a dependent can be holding a reviewed packet while its parent cannot publish. It does not reopen the Phase-A gate: a parent blocked *before* its dependent launches still skip-and-records that dependent, because Phase A is about building the dependent on a correct base while this rule is only about getting already-reviewed work to origin. Publish a dependent that cleared its own review gate anyway, onto the parent's existing remote tip, and reply and resolve its threads normally: the fixes are on origin, so the thread state is honest. Holding the dependent instead is the more fragile choice, because its reviewed work then survives only in a worktree that teardown may remove and a later rebase may strand, while its disposition context lives only in this session. Do not publish the parent's un-gated work on the dependent's behalf. Do not merge a dependent before its parent — an ordering this skill can only state, never enforce — and do not read any of this as discouraging a dependent PR that deliberately targets a feature branch rather than the default.
...
- [ ] Stacks detected only by matching fully qualified repository/ref pairs; any enabled rebase ran and published parent-before-dependent on a best-effort basis, with only an unchanged local tip equal to the recorded PR head eligible for the terminal zero-feedback shortcut, while changed or already-unpublished zero-item parents were reviewed and published first; a parent that became unpublishable after its dependent's wave launched blocked only itself, its already-review-gated dependents published anyway (the Phase-A skip-and-record above still governs a parent blocked before its dependent launches) with pings suppressed whenever `<base remote tip>..<pushed tip>` held anything that was not the dependent's own work and no bot had been named outright, and a merged parent counted as satisfied only after its final reviewed content was confirmed present in the refreshed base's current tree, failing closed when it could not be confirmed; dependents of setup-skipped, Phase-A-failed, or round-12-blocked parents were skip-and-recorded with the parent reason rather than rebased onto the original target; every rebase-enabled descendant of any changed parent tip was invalidated, restacked in topological order onto exact parent tips, re-reviewed, and checked for parent content in its diff.
