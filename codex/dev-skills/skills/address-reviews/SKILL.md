...
Once an entry's current work cycle has recorded its remote head OID, guard every later refresh the same way as the publication boundary below: fetch any newly observed exact head tip and require it to be an ancestor of local `HEAD` before adopting it as a new expected lease or continuing the cycle. If it is divergent, block the entry and its rebase-enabled descendants rather than silently replacing the lease expectation; only an explicit reconciliation that incorporates the remote tip followed by `delegated-fix`, the full fresh review gate, and descendant invalidation may resume it.
...
Immediately before that launch, re-fetch the PR and relevant refs and re-verify the same state, pair, and OID facts again. If the PR head OID changed, fetch that exact head ref without moving the local branch and require the refreshed remote tip to be an ancestor of local `HEAD` before adopting it as the packet's new expected lease or rerunning `delegated-fix`. A refreshed tip that is not an ancestor is concurrent divergent work: block the entry and its rebase-enabled descendants, and do not publish, force-push, or record that OID as replaceable. Reconciliation must explicitly incorporate the refreshed tip into the local branch, then rerun `delegated-fix`, the full fresh review gate, and descendant invalidation before publication; the hands-off batch never guesses this reconciliation. Invalidate the stale packet only when an entry-affecting fact changed: its head OID, open/merged state, fully qualified head/base pair identity, or dependency target because any parent tip changed; unrelated `baseRefOid` movement alone does not invalidate it. When a packet is safely invalidated, do not launch its publisher: rerun `delegated-fix` (including the required restack when a rebase-enabled dependent's parent changed its dependency target) and the fresh review gate first. If that rerun changes a parent's final tip for any reason, including an ordinary fix-up, immediately invalidate all already-reviewed rebase-enabled descendants and restack/re-review them in topological order before launching any descendant publisher. For a newly merged parent, pin the refreshed branch containing the merge to one exact commit and reuse it for every affected direct dependent; when no parent changed the dependency target, preserve the original exact non-parent target.
