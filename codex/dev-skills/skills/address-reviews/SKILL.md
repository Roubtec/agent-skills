...
- [ ] Stacks detected only by matching fully qualified repository/ref pairs; any enabled rebase ran and published parent-before-dependent on a best-effort basis, with only an unchanged local tip equal to the recorded PR head eligible for the terminal zero-feedback shortcut, while changed or already-unpublished zero-item parents were reviewed and published first; a parent that became unpublishable after its dependent's wave launched blocked only itself, its already-review-gated dependents published anyway (the Phase-A skip-and-record above still governs a parent blocked before its dependent launches) with pings suppressed whenever `<base remote tip>..<pushed tip>` held anything that was not the dependent's own work, and a merged parent counted as satisfied only after its final reviewed content was confirmed present in the refreshed base's current tree, failing closed when it could not be confirmed; dependents of setup-skipped, Phase-A-failed, or round-12-blocked parents were skip-and-recorded with the parent reason rather than rebased onto the original target; every rebase-enabled descendant of any changed parent tip was invalidated, restacked in topological order onto exact parent tips, re-reviewed, and checked for parent content in its diff.
