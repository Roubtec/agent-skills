...
- **Per-PR guidance comes from `address-review`, but the parallel orchestrator owns the phases.** A fix subagent runs `delegated-fix`; a separate fresh reviewer and concurrent best-effort `claude` peer check the returned packet; fix-up/re-review rounds follow as needed; only an entry that clears both verdicts gets a `publish-reviewed` subagent. The peer wiring in standalone `address-review` is not inherited because `delegated-fix` stops before review.
...
3. **Preflight the peer once for the batch, unless `peer-opinions=off`.** In this main working tree run `command -v claude`; when missing, mark the peer unavailable. Otherwise run `claude auth status`: success marks it available; on failure, a set `ANTHROPIC_API_KEY` or output showing that this older CLI lacks the `auth status` subcommand means classify availability at the first real invocation, while any other failure marks it unavailable. In a classify-at-first-invocation path, an auth/usage failure on the first real invocation marks the peer unavailable for the rest of the batch. Never repeat the probe inside entries. Unavailability does not block the batch; retain its reason for one final-summary note.
...
For each reviewer round, fan out one same-phase subagent per distinct worktree in one tool-call batch and launch that entry's peer beside it, wait for both verdicts for every entry, close the subagents, then advance the phase.
...
Before each launch, write `git -C <worktree> log --oneline <base>..HEAD` plus `git -C <worktree> diff <base>...HEAD` for that entry to the invocation's unique read-only artifact outside the entry worktree, alongside its unique outfile; never populate it from the orchestrator checkout or another entry. Include the artifact path in the prompt and grant its containing directory with `--add-dir`. Launch from the entry's exact worktree so concurrent entries cannot cross-read or interleave: `(cd <worktree> && claude -p "<prompt>" --tools "Read,Glob,Grep" --disallowedTools "mcp__*" --add-dir <artifact-dir> --output-format json [--effort high] > <outfile> 2>&1) &`. Keep both read-only guard flags after the prompt, never pass permission-bypass flags, and omit `--effort high` only when the configured effort is already known to be `high` or `xhigh` so a stronger default is not downgraded. Use a loose roughly 12-minute timeout, extending it when review size warrants; retry a timeout or transient failure once with fresh per-attempt files, then forfeit that entry's opinion for the round. An auth/usage failure on a classify-at-first-invocation attempt makes the peer unavailable for all later entries and rounds. `claude -p` may emit nothing until completion; wait for both the Reviewer and peer before deciding this entry's outcome.
...
- [ ] Per-PR phases ran in order: `$address-review ... delegated-fix`, fresh `explorer` Reviewer plus concurrent best-effort peer, fresh `worker` fix-up/re-review as needed (6 reviewer rounds max), then `$address-review ... publish-reviewed` only after both outcomes cleared the gate unless the run is `no-push`; distinct heads fanned out concurrently but throttled; same-head entries serialized.
