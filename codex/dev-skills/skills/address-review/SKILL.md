...
4. **Per-thread hygiene** — for each triaged thread still unresolved (recipes below):
   ...
   - *Follow-up-task* → reply with the placement-specific template: queued work uses `Follow-up task committed: tasks/NNN-… — queued for an upcoming batch`; parked work uses `Deferred with full context to tasks/deferred/NNN-… — <the condition it waits on>`. **Resolve** when the follow-up was maintainer-directed or the thread is bot-authored; leave a human-authored thread unresolved unless the maintainer authorized closing it. Never re-implement a task-backed thread.
