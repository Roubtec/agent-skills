...
One thing can still **falsify** the maintainer's statement cheaply, so it is checked: where the named branch is itself the head of another **open** PR (`gh pr list --head <branch> --state open`), its commits belong to that PR rather than to this one, and a PR found there is a stop rather than a mode — ask which PR was meant (hands-off: halt naming both branches and both PRs). That is the reverse of how the deleted heuristic used the same probe. As a recognizer it needed "no open PR" to imply "off-shoot", which it does not; as a falsifier of a claim already made, an open PR on that head is direct evidence against the claim, and what the probe misses no longer decides anything.
...
- A same-named local ref failing the PR identity check, a named off-shoot that is itself another open PR's head, `off-shoot` given on a detached HEAD, or a reused worktree holding a prior run's dirt or half-finished operation → halt with the report "Working location" defines (both refs and what each points at; both PRs for the branch carrying its own; the current `HEAD` for the detached case; or the worktree path and what it holds). Attach nothing, substitute no branch name, clean nothing.
