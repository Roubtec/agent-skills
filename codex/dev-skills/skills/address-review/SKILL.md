...
**Arguments:** `[PR#] [rebase on top of <branch>] [no-rebase] [inline] [off-shoot] [no-push] [push] [hands-off] [peer-opinions=off] [ping-codex] [ping-claude] [ping-copilot] [ping-contributing]`
...
| `rebase on top of <branch>` | Rebase the **working location's branch** onto `<branch>` rather than onto the PR's own base, which is what step 2 rebases onto by default. Single-branch rebase only. |
| `no-rebase` | Opt out of the default rebase entirely — **both** of step 2's points, so the branch is addressed and pushed on the base it already sits on. Use it when the base is known to be moving under you, or when a rebase would obscure what a reviewer is being asked to look at. It wins over `rebase on top of <branch>` if both arrive. |
...
- **`hands-off` + the default rebase** is the ordinary unattended combination now that rebasing is not opt-in, and its one sharp edge is a conflict nobody can be consulted about: the delegated rebase aborts cleanly and the run stops with that open question rather than guessing (see "Hands-off mode" and step 2). `no-rebase` is how an unattended run avoids the edge entirely.
...
- **Rebaser** (default, once per rebase point) — a fresh subagent that performs one of step 2's rebase points under `review-cycle` → "The delegated rebase step", and hands back the pinned effective base, the before/after tips, and any halt. It is the only role here that rewrites history, and it is never also the fixer.
...
`no-rebase` suppresses **both** points, and nothing else does. `rebase on top of <branch>` replaces the default target with the one it names. Given both, honor `no-rebase` and note the ignored token, exactly as `no-push` wins over `push`.
...
- **The target.** By default the PR's own `baseRefName`, fetched fresh at the point the rebase runs rather than read from a remote-tracking ref some earlier step updated. `rebase on top of <branch>` names a branch or an exact commit instead (a batch orchestrator pins one commit so every entry rebases onto the same base). Either way this is a **single-branch** rebase: a whole chain of dependent branches is the separate `rebase-stack` skill — mention it if the user seems to want chain-wide restacking. Where this PR's base is another in-flight PR's head, the target is that parent's new tip, per the delegated step's parent-first rule.
- **Where it runs.** The working location, like everything else after step 1. Inline that rewrites the branch the maintainer is standing on, which is mode 1's accepted contract; in worktree mode the rewrite and any conflict happen in a tree that is disposable, and the main checkout is unaffected either way.
- **Both points sit inside steps 0–5**, the second once every intended change is committed — so a `delegated-fix` run carries both and returns a packet whose final SHA is the rebased tip, and a `publish-reviewed` run rebases nothing.
...
- **The second point runs before the reviewer round**, so a passing verdict always describes the exact tree step 7 pushes. Where it replayed anything, its delivery-tier validation runs and step 6 reviews the rebased tree; where it was a no-op, the tree the reviewer sees is already the one being pushed.
- **A halt is a stop, not a guess.** A conflict beyond the delegated step's competence comes back as an open question with the tree left clean and idle. **Interactive:** put that question to the user — the conflict, your proposed resolution, and the reasoning — and hand their answer to a *fresh* rebase subagent rather than finishing the rebase in your own turn; loop them in as many times as needed. **Hands-off:** the run stops there having pushed nothing, with that open question and its conflict detail (files, offending commit, why) as the blocker — addressing review on a wrong or stale base and then force-pushing is worse than not running.
...
- Non-trivial rebase conflict → the delegated rebase aborts cleanly, leaves the tree clean and idle, and the run stops with it as an open question (step 2). Nothing is pushed on the old base instead.
...
- [ ] Both of step 2's rebase points ran unless `no-rebase` was given (or a point was judged unnecessary and said so), each delegated to a fresh subagent rather than run in your own turn, each single-branch, with the effective base pinned to the OID actually rebased onto; a conflict beyond that step's competence halted it with the tree clean and idle (interactive loop-in / hands-off stop), and a rebase that replayed anything was validated.
