...
**Arguments:** `[PR#] [rebase on top of <branch>] [inline] [no-push] [push] [hands-off] [peer-opinions=off] [ping-codex] [ping-claude] [ping-copilot] [ping-contributing]`
...
| `inline` | Force **inline** working — do the work in the current checkout rather than in a worktree, checking the target branch out there if it is not already a working location for this PR. Requires that checkout to be clean, and the run ends on the target branch. Unnecessary when you already stand on the branch (that is mode 1 anyway); see "Working location". It composes with `no-push` and the ping flags without interacting with either. |
...
- **`inline` selects the working location and nothing else.** It does not imply or suppress a push, name or drop a ping, or change how anything is triaged; it is orthogonal to every row of the table above and to `hands-off`. The one thing it does change is what the preflight requires — an inline run needs the current checkout clean (see "Working location").
...
**The PR identity check** is step 1's PR-vs-branch sanity check, applied to whichever branch is a *candidate* working location rather than only to the one you happen to be standing on: the candidate shares recent history with the PR head, and for a fork PR its resolved push remote/ref matches the PR head repository/ref (the same resolution step 7's publication preflight performs). A branch that fails it never becomes the working location — not by being checked out, not by being attached.
...
3. **Forced `inline` — the argument.** It forces the work into the current checkout. Where that checkout is not already a working location for this PR, check the target branch out there (creating a local tracking branch from the PR head when there is none — `gh pr checkout <N>` for a fork head, which wires the fork remote and tracking for you) and proceed as mode 1. The run ends on that branch — the maintainer asked for the checkout, so it is not restored.

A name match alone never selects mode 1. A same-named local branch that fails the identity check falls through to mode 2, and there into the collision stop below — never *onto that branch*. Reusing a name like `minor-fixes` or `batch-wrap-up` across unrelated work is ordinary maintainer practice, so treat that collision as expected rather than exotic.

**The one standing exception to mode 2** is the local off-shoot this skill already supports: an explicit `PR#` was supplied for an **open** PR, and the current checkout is on a named branch that is not the PR's base ref and already **carries the PR head** in step 1's exact sense (every commit unique to the head is represented in it, by ancestry or patch-id). That branch is a working location the maintainer chose, so the run works inline on it — fixes land there, `headRefName`/`headRefOid` stay publication metadata, and step 1's reconciliation is skipped rather than used to drag the off-shoot onto the head. A detached HEAD is not covered by this exception: it takes mode 2 whatever it carries.

### Preflight in the working location

Step 0's clean-tree and no-operation-in-progress checks are checks on **where the work will happen**, not on the main checkout:

- Inline (mode 1, the off-shoot exception, and forced `inline`) keeps today's requirement whole: that checkout must have an empty `git status --porcelain` and no Git operation in progress.
- Worktree mode requires nothing of the main checkout, which may be dirty or mid-anything. A worktree created for this run is clean by construction. A **reused** one — the stable slug surviving a prior halted run — gets exactly the same two checks, and dirt or a mid-rebase state there is a stop-and-report naming the worktree path: it is most likely that prior run's remains, and this run neither cleans nor forces past it (hands-off: document and stop).

Either way, nothing is auto-stashed or discarded anywhere; dirt is the maintainer's to resolve.

### Attaching the worktree

Branch resolution and attach follow `address-reviews` → "Resolving and checking out each entry" wholesale — local-first (a local ref wins over `origin`), the `origin` head only when no local branch exists, and a fork PR through its detached-worktree + `gh pr checkout` case, whose landed-on-the-right-head verification is part of that recipe. Prefer `wt-enter <slug> <branch>` where the worktree helpers are on PATH, with a stable slug (`pr-<N>`) so a halted run's worktree is found again rather than duplicated; else the plain `git worktree add` those rules describe.
...
1. **Clean tree and no rebase in progress — checked in the working location, not here.** Both checks survive unchanged in substance but are scoped to wherever this run will work, which "Working location" picks once step 1 has resolved the PR; run them there. An inline run is therefore still held to a completely clean current checkout (`git status --porcelain` empty) with no rebase in progress (`git rev-parse --git-path rebase-merge`, `--git-path rebase-apply`), and stops to ask the user to commit/stash/clean it (hands-off: document and stop); a worktree run requires nothing of the main checkout and applies the same two checks to a reused worktree. Never auto-stash or discard files, wherever you check.
...
In worktree mode, reclaim the worktree first, per "Working location" — after publication and equally after a `no-push` finish. A run that is halting rather than finishing keeps its worktree and reports the path instead.
...
- A same-named local ref failing the PR identity check, or a reused worktree holding a prior run's dirt or half-finished operation → halt with the report "Working location" defines (both refs and what each points at; or the worktree path and what it holds). Attach nothing, substitute no branch name, clean nothing.
...
- [ ] `gh` authenticated; the working location's tree clean and no rebase in progress there — the main checkout held to that only on an inline run.
- [ ] Working location picked per "Working location" before any reconciliation: inline on the target branch or the supported off-shoot, a worktree from anywhere else (detached included), the current checkout when `inline` was given. A candidate branch failing the PR identity check was reported to the maintainer with both refs and never worked, checked out, or attached; nothing invented a branch name in its place.
