...
**Destination interface — `peer-review-run`, and nothing powbox owes still blocks this side.** powbox bakes a provider-neutral runner, `peer-review-run`, that owns literal prompt-file handling, read-only execution, timeout, retry, and reaping, and reports the outcome vocabulary above as machine-readable JSON (schema `powbox.peer-review-run/v1`: the final stdout line is one JSON object; `outcome ∈ passed | issues | unavailable | timeout | forfeited | failed`; exit 0 for any produced outcome). It is this step's baseline interface, and for the **claude** peer this codex-led side launches it can carry the pin today: the helper takes `--model` and `--effort`, passes both to `claude` — dropping `--effort` and reporting `effort: null` only on a CLI whose probe finds no such flag, an honest, visible degradation rather than the raw launch's hard failure on an unknown flag — and reports the strength it actually applied as `model` and `effort` in its result. So this step should move onto the helper, gaining its timeout, retry, and reaping — passing `--model opus --effort medium` explicitly, `--effort` in particular because the helper's own default is `high` and this pin is `medium`. That move is task 015's swap, in the repos that track these skills' tasks; the pinned raw launch above is what this step documents until the swap lands, and it waits on that swap alone rather than on anything powbox owes. The one outstanding half of the passthrough is codex-side and constrains the Claude-led mirror, not this one: the helper's codex adapter passes `--ignore-user-config`, discarding the `~/.codex/config.toml` that a codex peer's configured high-capability model comes from, and its `--model` default (`opus`) applies to the claude provider only, so a codex peer launched through it reviews on the CLI's bare default model.
