...
In normal interactive mode, wait for typed `go` or a branch list. Apply keep/skip changes, re-display the updated listing, and require a final `go`. If the user explicitly requests deletion of an Uncertain branch, mark that row `explicitly confirmed`, include it in the updated listing, and require the final `go`; it receives a recovery ref.
