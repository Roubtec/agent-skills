...
- Delete each with `git update-ref --no-deref -d "$ref" "$old_oid"`, passing the OID step 5 inventoried as the expected old value. Git refuses whenever the ref no longer holds that OID; that refusal is the entire reason for the expected-old value, so record it as a skip and never retry without it. A refusal is informative in its own right whatever its shape, but only the probe below separates one from an ordinary failure that left the ref at its inventoried OID — lock contention being the usual one, and it reports `cannot lock ref` too. Once the probe has attributed the failure to the expected-old check, it establishes that something outside this run moved, deleted, or replaced a ref under `refs/pruned/**` while the run was in progress, which the user should hear about regardless of what else the report says about that breadcrumb.
