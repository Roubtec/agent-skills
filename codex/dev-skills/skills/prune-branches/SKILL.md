...
This pin costs nothing for the two tip-reachability cases: a branch tip reachable from the freshly fetched remote default is already Merged by ancestry before these rules run. It does bite the merge-parent case, which tests parents rather than a classified branch tip. A parent that is on origin's default but not on a local default lagging behind it, or that was never created because `no-pull` or a failed temporary worktree skipped its creation, fails that limb of the proof, so the merge-parent case no longer proves Transient. Another surviving ref covering the parent, or any other Transient case, still can. Failing all of them the branch is Uncertain and therefore kept, which is the conservative direction, and pulling or creating the local default restores the proof on a later run.
